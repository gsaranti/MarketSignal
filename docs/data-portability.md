...
3. Read and validate the manifest: reject a format version newer than this build understands; verify every entry's size + checksum, and never consume bytes the manifest doesn't list.
   Every table entry **the archive's own format version requires** (five in format v1; six in v2, which added the quick-check store with its landing slice; eight in v3, which added the outcome-episode store and the price-bar cache with the outcome-learning slice) must be **present and manifest-listed** — a truncated archive is refused, never imported as a sparse store, while an older archive imports complete under *its* version's entry set (backward compatibility by version, never sparse tolerance).
...
- **Local-suite namespaces** (`portfolio`, `opportunities`) embed with the local model.
  The manifest records the **per-namespace embedder id**, and import compares **model identity, never dimension** — two different models can share a dimension, so a dimension match proves nothing.
  On an identity mismatch the imported vectors are never cosine-compared against the new space: they are queued for **re-embedding from their retained `content`** (harmlessly skipped until then — a differing dimension is additionally skipped at search time), so the content always survives and the mismatch degrades to a re-embed, never data loss ([storage.md §Local Vector Memory](storage.md#local-vector-memory)).
...
Local-suite coverage is a **format-extension rule, not an automatic property**: every new durable local-suite store — the [storage.md §Local Analysis Suite Storage](storage.md#local-analysis-suite-storage) set (Portfolio outcome episodes + their matured archive; the opportunity matrix / run, opportunity graph, departed archive, shadow ledger and its matured archive, and the picked-episode store and its matured archive) — **joins the archive as part of the slice that lands it**: a new required manifest entry and a format-version bump, with regenerable caches staying excluded under the what-moves rule above.
Format v3 records the one exception to that cache exclusion: the shared **price-bar cache rides with the outcome episodes** (the table above), because imported pending episodes must be able to mature offline rather than refetching a year of bars — the cache is regenerable in principle but load-bearing for the episodes it travels with.

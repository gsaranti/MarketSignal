...
| `portfolio_runs` | Local-suite run history (retention 30; the priced verdict's target-rationale field renamed with its meaning in format v7; each ledger condition carries the model's `label` beside an app-rendered statement since format v8). Nascent today, but durable once the suite runs live. |
...
3. Read and validate the manifest: reject a format version newer than this build understands; verify every entry's size + checksum, and never consume bytes the manifest doesn't list.
   ...
   The pre-release v2 through v6 shapes, which no shipped build wrote, are refused outright (ruled 2026-08-29 — no local-suite data compat pre-release; [verification/2026-08-29-fresh-start-2-local-suite-compat-removal.md](verification/2026-08-29-fresh-start-2-local-suite-compat-removal.md)).

...
The note the synthesis reads is in plain words — how many searches returned nothing, how many pages could not be retrieved or are shown truncated, and whether searching was stopped before it finished — while the persisted gap keeps the mechanical summary (the cap, bound or budget that stopped it) for diagnosis; the two render from one record (ruled 2026-09-17).
A pass that retrieved no page with body text spends no synthesis call: the app records the pass itself — a fixed sentence plus the searching note, no claims, no follow-up — with the same gaps (fix list 4.3, ruled 2026-09-17).
The synthesis message closes with the findings object's shape, every value a placeholder and the source id listing the ids the evidence shows, pinned to the grammar's key set by test.

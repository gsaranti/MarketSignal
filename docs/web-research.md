...
The shipped `settings.yml` also curates the engine set, re-tuned against live-run observations ([verification/2026-08-30-big-run-findings.md §Finding 1](verification/2026-08-30-big-run-findings.md)): it keeps the general engines that returned usable results under sustained load (`google cse`, Reuters, DuckDuckGo, Mojeek) and disables the ones that CAPTCHA or rate-limit on effectively every automated call (Qwant, Startpage, Brave's HTML scraper, and Google's HTML endpoint).

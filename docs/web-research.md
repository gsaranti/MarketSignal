...
**Context stays bounded by condensing as it goes.** Local models have finite context, so the loop never accumulates raw pages or the full turn-by-turn transcript. Each batch of fetched, extracted results is condensed by the reasoner in non-thinking mode (the fast 35B tier when resident — see [local-models.md §The model roster and per-task routing](local-models.md#the-model-roster-and-per-task-routing)) into structured **findings notes**; the working context the reasoner plans from is the *running condensed findings plus the current batch*, not everything seen so far. Alongside the rolling notes the loop keeps an **append-only evidence ledger** — each extracted claim with its source URL and timestamp — that condensation summarizes *from* but never overwrites, so repeated summarization can't silently drop conflicting or contrarian evidence; the ledger feeds the run's audit record. The loop stops when the agenda's questions are answered or the budget is spent, and the condensed findings — each carrying its source URL and timestamp — are what flow downstream (the "forward only what's needed" rule from [local-models.md §Context-memory discipline](local-models.md#context-memory-discipline), applied inside research).

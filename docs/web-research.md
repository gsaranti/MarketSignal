...
Within a topic's bounded pass the reasoner works over that topic's extracted page text and the ledger; older raw page text may roll off the working context as the pass proceeds, but the ledger retains the claims, so nothing is silently dropped.
...
The orchestrator decides single-vs-hierarchical **deterministically** — the accumulated evidence-ledger size against the consolidation call's input budget — never the model; the thresholds are config knobs ([configuration.md §Local Analysis Suite Configuration](configuration.md#local-analysis-suite-configuration)).
...
The shipped config also pins the general engines that don't aggressively CAPTCHA automated queries (e.g. Brave, Mojeek, Qwant, DuckDuckGo) and binds to loopback; Google is deprioritized because it CAPTCHA-walls hardest.
The app **health-checks** the running instance — the same health-check *mechanism* it uses for the model daemon (see [local-models.md §Serving runtime](local-models.md#serving-runtime)), but **never on the execution gate**: an unreachable SearXNG degrades the run (fall back to Tavily, or fewer discovery candidates — [§Tavily fallback](#tavily-fallback)), it does not block it.
...
- **SSRF protection.**
  Fetches are restricted to `http`/`https` and to public hosts — private, loopback, and link-local address ranges are blocked (this matters specifically because the app's own Ollama and SearXNG run on loopback), redirects are capped and re-validated against the same rules, and responses are bounded by size and content type (HTML/text only).
- **Untrusted content.**

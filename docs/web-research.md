...
- **cost-free and unlimited** — a local instance has no per-query credit ceiling, which a deep multi-step research loop over many items would otherwise exhaust;
- **local-first** — no key and no third-party dependency in the default path;

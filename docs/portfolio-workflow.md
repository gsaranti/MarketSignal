...
| — | Outcome-learning pass (after 7b; persists with Step 8) | Computed (engine) + API retrieval (label-time dated-EOD bars + dividends) | — |
...
**Type:** API retrieval (FMP / SEC EDGAR / FINRA) + Computed (assemble the packet); the local-model embedding call for semantic continuity retrieval is **designed, not built**.
...
The response schema offers the **full ladder to every holding** (the target-weight fields keep their structural `minimum: 0`); where a final action departs the standalone lean, the nullable `divergence_cause` is validated against the same aggregates — an omitted cause annotating the departure as unattributed, a checkability-failed cause named once for repair and stripped on merged-repair residue — with the `engine-bar` and `carried-stale-lean` cases app-stamped instead ([portfolio-analysis.md §Portfolio roll-up and construction](portfolio-analysis.md#portfolio-roll-up-and-construction)).

...
Where the pre-profit overlay matched a Medium or Low ceiling, the model receives that ceiling and must interpret the execution evidence beneath it; where severe deterioration matched, its standalone lean must be one of the engine-provided `{trim, sell all}` choices.
...
A tripped **hard forensic trigger** (restatement / auditor change / fraud flag — the trip resolving only from the shared typed producer: an item-classified filing kind, or a validated `forensic_event` research claim from 6d, never a bare model assertion — [trade-opportunities-workflow.md §Step 5c](trade-opportunities-workflow.md#step-5c-deterministic-analysis-archetype-weighted-engine)) is the one case with its own stronger clamp — **conviction hard-capped at Low**, the same after-the-raise min formula with the lower ceiling, the matched hard rule recorded exactly like a soft clamp — and the disposition's remaining legs are enforced where they live: the add-family bar in Step 7a's feasible-set bounding, the exit-tilted standalone lean validated here like any other verdict move ([portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable) is the canonical hard outcome).
The pre-profit overlay's rule consequences are validated by the same app-owned pattern, but remain separately attributed from forensic rules: a repeated execution miss contributes a **Medium** ceiling, severe deterioration contributes a **Low** ceiling, and the strictest matched ceiling binds after any validated raise; a constrained-runway or severe state is carried into Step 7a's add-family bar, and a severe state rejects a 6f standalone lean outside `{trim, sell all}`.
The validator accepts those consequences only from the Step-6e engine state assembled from app-validated observations; model prose cannot create an execution miss, runway state, or severe conjunction.
...
The response schema is **narrowed per holding**, mirroring the 6f narrowing: each holding's `action` enum lists exactly its offered set (a barred rung is structurally unreachable) and the target-weight fields carry a structural `minimum: 0`; where a final action departs the standalone lean, the required `divergence_cause` is validated against the same aggregates, with the `engine-bar` and `carried-stale-lean` cases app-stamped instead ([portfolio-analysis.md §Portfolio roll-up and construction](portfolio-analysis.md#portfolio-roll-up-and-construction)).

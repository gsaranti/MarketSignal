...
| 5 | Load shared context (house view, profile, run-level FRED rates, commodity context, CFTC positioning, CBOE backdrop, sector benchmarks) | Computed (local read) + API retrieval (FRED / FMP / CFTC / CBOE) | — |
...
**Type:** Computed (local read — house view, investor profile) + API retrieval (run-level FRED rates, the FRED/FMP commodity context, CFTC positioning, the CBOE put/call backdrop, and the sector-benchmark series).
No model.
...
**(b)** A conviction layer → the **momentum / market-setup read**, the **hard forensic state** (the hard-event kinds reading the shared typed producer — as-built the item-classified EDGAR filing kinds; the research-fed `forensic_event` fraud claim joins when the loop lands — [trade-opportunities-workflow.md §Step 5c](trade-opportunities-workflow.md#step-5c-deterministic-analysis-archetype-weighted-engine)), and — designed, landing with their data legs ([portfolio-analysis.md §The per-holding pipeline](portfolio-analysis.md#the-per-holding-pipeline)) — the narrative-vs-reality ratio (with its thin-coverage fallback), the soft **forensic flags**, and the **implied-expectations read** (the shared Step-5c primitive), all kept *out* of the letter.
...
The narrative-vs-reality and soft-forensic reads and the **implied-expectations range** join when their layer-(b) producers land; the **same-underlying option overlay** reaches no prompt as-built — its dossier leg is designed, unbuilt, and its former stand-in retired with the construction spine ([portfolio-analysis.md §The per-holding pipeline](portfolio-analysis.md#the-per-holding-pipeline)).
...
As-built the **severe-deterioration** Low ceiling and the **hard-forensic** trip can bind: the first's legs are statement-derived, and the second's filing kinds read the item-classified EDGAR sweep run at dossier assembly.

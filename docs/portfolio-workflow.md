...
| 6 | **Per-holding analysis loop** (per eligible holding, checkpointed) | mixed — see 6a–6g | 122B |
...
| 8 | Persist run & audit + memory embeddings (as-built matured learnings only; per-holding verdict embeddings designed) | Computed (persist) + Local-model (embedding) | Qwen3-Embedding-4B · fixed |
...
**Option chains are fetched fresh within the run, per holding at its Step-6 dossier assembly** (so a selective run's carried tail spends no chain call), from `GET /marketdata/v1/chains` — per-contract volume, open interest, IV, and greeks — bounded by expiration and strike range; the **shared freshness bound** that would reject a stale chain (mirroring the report's COT freshness guard) is **designed, not built** — as-built no as-of timestamp is retained and no staleness rejection runs ([schwab-integration.md §What is pulled](schwab-integration.md#what-is-pulled)).
...
- the **investor profile** (risk tolerance, horizon, objective, tax sensitivity, available cash / buying power — see [configuration.md](configuration.md)) — reaching **no model call before Step 7b construction**, so the intrinsic verdict is profile-independent; the loop's one read is mechanical, the engine bounding a sizing delta by available cash ([portfolio-analysis.md §Intrinsic verdict](portfolio-analysis.md#intrinsic-verdict));
...
**Type:** API retrieval (FMP / SEC EDGAR / Stooq / FINRA) + Computed (assemble the packet); the local-model embedding call for semantic continuity retrieval is **designed, not built**.
...
It adds any **same-underlying option positions** from the Step-2 pull — deterministic symbol link, designed as a **typed overlay** (direction / quantity / strike / expiry / delta / coverage ratio, classified covered-call / protective-put / collar / other); **as-built the overlay is classified at the deterministic Step-7a stand-in instead**, no dossier leg and no per-leg delta ([portfolio-analysis.md §The per-holding pipeline](portfolio-analysis.md#the-per-holding-pipeline) Step 1).
It adds the symbol-scoped **`news/stock`** headlines as research-loop **seeds** (leads, never evidence — [web-research.md §The research loop and context management](web-research.md#the-research-loop-and-context-management)).
...
It adds price history (Stooq) and a live quote (FMP `quote`); the prior run's verdict **and thesis ledger** for this holding ([portfolio-analysis.md §The position thesis ledger](portfolio-analysis.md#the-position-thesis-ledger)); and the Step-5 shared context.
...
For a priced stock meeting the deterministic eligibility rule, layer (b) additionally computes the structured half of the **pre-profit execution / financing overlay** ([portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable)).
...
It emits the provisional financing state and the statement-derived economics / dilution legs with every missing input typed `unscorable`; funds and `role_risk_only` holdings skip the overlay.
The non-standardized operating observations — production, deliveries, bookings / backlog / reservations, guidance, and unit economics — do not exist yet at this stage and therefore cannot be guessed; Step 6e validates any rows research later supplies and asks the engine to finish the overlay.
...
- when research validated a **countable, dated, third-party leading indicator the structured feeds did not carry** (a `research`-class signal), a typed **`validated_leading_indicator`** — `{ metric name, value / level, direction (inflecting up / down), as-of date, source URL, confidence, the thesis-ledger key driver it confirms }`.
  It is **distinct from `research_forward_assumption`** (which moves targets); its conviction-raise citation role is **retired with `portfolio-v7`** — conviction is the model's own ([§Step 6f](#step-6f-model-interpretation)), so the typed signal reaches the model as ledger-driver evidence with no gate riding on it (Trade Opportunities retired the same machinery on the same reasoning, so the retirement is suite-wide — its **cap** contract stands, [trade-opportunities.md §The opportunity](trade-opportunities.md#the-opportunity)); and
...
For an overlay-eligible stock, the app separately validates every `pre_profit_execution_observation` against the metric kind, polarity, numeric value, units, period, and issuer scope, rejects an unmatched or non-comparable row with its reason, and merges accepted rows into the period-keyed observation history without duplicating a previously stored observation.
...
The engine joins those reads to the Step-6b runway, margin, capex, and dilution values and derives the financing, economics-deterioration, material-dilution, and severe-deterioration states under [portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable).
The resulting rule consequences are also deterministic: repeated execution miss → Medium conviction ceiling; constrained runway → add-family bar; severe deterioration → Low conviction ceiling + add-family bar + an exit-family-only (`{trim, sell all}`) action rule — since `portfolio-v7` all three bind the **engine arm** (its stand-in conviction and action observe them) and reach the model as rendered engine rules, the model's own conviction and lean unrestricted with departures annotated.
...
The engine's deterministic values ride as the **baseline arm**, and this call authors the **model arm**: its own four sub-scores (the model letter derives app-side through the shared cutoffs), its own freely-authored one-/twelve-month target bands, conviction, the horizon reads, a retrospective `self_assessment`, and the **standalone action lean** on the full ladder — with the engine's numbers in the prompt as evidence, never as bounds.
...
**Prompt — input.**
The engine's computed analysis reaches the prompt: sub-scores, the refined scenario targets with exposed methodology **and typed provenance** — the `TargetMeta` derivation flags (rate-anchored vs current-multiple carry, flat / clamp-flattened driver, dispersion floor) rendered into the prompt so a low-signal target surface is weighed, not obeyed — any finalized **pre-profit execution / financing overlay** with its rule-bounded conviction ceiling and intrinsic-lean set, and the options-activity signal.
The narrative-vs-reality and forensic reads and the **implied-expectations range** join when their layer-(b) producers land, and the **same-underlying option overlay** as-built reaches the Step-7b construction prompt via the 7a stand-in, not this call ([portfolio-analysis.md §The per-holding pipeline](portfolio-analysis.md#the-per-holding-pipeline)).
The prompt also carries the distilled research findings and the house view — **scoped to the horizon reads and market-setup context**, never by itself a per-holding exit reason (the investor profile is **deliberately absent**: the intrinsic verdict is profile-independent and the profile enters at Step 7b only — [portfolio-analysis.md §Intrinsic verdict](portfolio-analysis.md#intrinsic-verdict)).
...
A matched pre-profit conviction ceiling and a severe-deterioration lean restriction reach the model as **prompt evidence and persist as audit annotations**; neither narrows the schema nor clamps the returned value, and the engine arm's own conviction and action observe them by construction.
...
A **standalone action lean** on the fixed ladder (sell all → trim → hold → add → add aggressively) comes *before* portfolio context: a **fails** hurdle read is a **weighed exit input** on the holding's own merits, set against the targets' provenance and the data quality ([portfolio-analysis.md §The holding verdict](portfolio-analysis.md#the-holding-verdict)).
A financial-health read joins it, plus the **rewritten thesis ledger** — thesis, bear/base/bull monitor, falsifiers, triggers, each **quantitative** falsifier and trigger stated **machine-evaluably**: the engine series, comparator, threshold, and persistence semantics, validated at Step 6g.
Last is the **intrinsic half of the what-changed audit**: every moved intrinsic value (grade, sub-score, conviction, target, horizon, a re-weighted scenario, a tripped falsifier / fired trigger) shown old → new with its cause **attributed** to an *external* change (market data / company information / research-narrative, each tied to the engine's input delta or a research finding), to a **labeled self-correction** where the inputs did not move, or — when the prior verdict's stamped `grade_parameter_version` differs from the current bands — to the **band recalibration**, never mislabeled as evidence or self-correction.
...
The engine-matched **cap rules persist as annotations that bind the engine arm**: a tripped hard forensic trigger (restatement / auditor change / fraud flag — the trip resolving only from the shared typed producer: an item-classified filing kind, or a validated `forensic_event` research claim from 6d, never a bare model assertion — [trade-opportunities-workflow.md §Step 5c](trade-opportunities-workflow.md#step-5c-deterministic-analysis-archetype-weighted-engine)), a repeated execution miss's Medium ceiling, and severe deterioration's Low ceiling each record on the audit and bound the **engine stand-in arm's** conviction, action rule, and Step-7a engine set — while the model's conviction and lean persist as authored, the departure rendering beside the recorded rule ([portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable) stays the canonical hard-outcome record).
...
The engine computes the deterministic whole-book picture: concentration and sector / factor exposure (**fund exposure folded in at the sector level** — single-name look-through is off-plan with `etf/holdings`, so direct-plus-fund overlap aggregates at the exposure level unless SEC N-PORT supplies constituents), correlation / overlap clusters, and the cash / buying-power position.
It computes **the risk / exposure contribution of any material not-rated positions** — market value + signed notional, the payload-derivable inputs; fixed-income duration / credit and a standalone option's delta ride as **typed gaps**, no on-plan source carrying them, the held-underlier overlay delta from its own chain the exception ([portfolio-analysis.md §Portfolio roll-up and construction](portfolio-analysis.md#portfolio-roll-up-and-construction)) — graded nowhere but real exposure.
Per holding it computes the **action-sizing spine inputs**: existing weight, concentration headroom against the profile's limits, overlap contribution, the upside/downside the targets imply against price, the **capital-efficiency / dead-money** read (total-return basis), any **same-underlying option overlay**, the position's **unrealized P/L** (a harvestable loss or a taxable gain, by sign), and the risk tier (assigned and persisted at Step 6b — [portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable)).
The spine inputs also carry the **hard forensic state** (a tripped restatement / auditor-change / fraud trigger bars the add family — the canonical hard outcome, [portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable)), any **pre-profit overlay action rule** (constrained runway or severe deterioration bars the add family from the engine set; severe deterioration also bounds the engine arm's lean stand-in to the exit family — engine-arm rules since `portfolio-v7`, the model's lean unrestricted), and tax.
These **bound the engine action set** — since `portfolio-v7` the bounds bind the engine arm alone, reaching the 7b model as the engine's rendered read, an outside-the-set choice persisting with an engine-bound annotation, never a schema bar — the grade tests in those bounding rules reading the **ex-momentum composite** ([portfolio-analysis.md §The holding verdict](portfolio-analysis.md#the-holding-verdict), [§Starting parameters](portfolio-analysis.md#starting-parameters-calibratable)).
A **`role_risk_only`** holding supplies the **reduced spine**, its engine set **{sell all, trim, hold}** ([portfolio-analysis.md §Portfolio action](portfolio-analysis.md#portfolio-action)).
The deterministic half of the **outcome-learning pass** runs as its own step **after the 7b construction merge**, not inside this one — episode planning needs the merged final actions and divergence records, so the alignment tags, label maturation, episode append, and scorecard reads land in one seam and persist atomically with Step 8.
In that pass the engine tags each active decision episode's **`observed_net_alignment`** from the Step-4 diff and computes any newly **matured window labels** after refreshing the episode symbol's Stooq bars through the window end — including symbols no longer held — and re-pulling its `dividends` history for the total-return leg (the pass's own label-time API retrievals).
A failed Stooq refresh uses cached bars only when they cover the full window; otherwise the label remains pending with a coverage gap, bounded by the shared price-coverage grace — past it the leg closes typed, the grace doubling as the transient-vs-disappearance discriminator: a symbol the source never covered takes **`price-coverage-unscorable`**, while a series that covered the entry and then stopped resolves per the terminal contract ([storage.md §Local Analysis Suite Storage](storage.md#local-analysis-suite-storage)).
It then appends-or-extends this run's own episodes (7b's final actions in hand) and derives the scorecard reads over the updated set — label mechanics, bases, and cohort layers all per the canonical contract in [portfolio-analysis.md §Outcome learning](portfolio-analysis.md#outcome-learning-calibration); fresh episodes carry no matured windows, so the reads move only on matured history.
...
**Prompt — input.**
All per-holding **intrinsic** verdicts and standalone action leans (a `role_risk_only` verdict carries no lean — its action is authored wholly at 7b, the reduced set its engine set); the Step-7a whole-book aggregates and per-holding sizing-spine inputs (with the engine's own feasible-set read); the **exited** names from the Step-4 diff; the house view; and the investor profile.
Each holding's digest carries its **ENGINE SET with every rung's numeric engine band** (decimal fractions of the book — the same units the response contract requires, stated in the system prompt) — since `portfolio-v7` rendered as the engine arm's own read, **evidence the model weighs, never a bar**: the full ladder is open on every holding, and a departure records as an engine-bound annotation beside the plan — plus the single-position concentration cap, the prior action / prior lean baselines the attribution rules compare against, and — for a `role_risk_only` holding — its class label, role read, expense drag, observable risk, exposure tilt, structural flag, and evidence gaps, since 7b is that branch's sole action author.

**Returns.**
Each holding's **final action** (fixed ladder) and **target-weight range / share-dollar adjustment** are reconciled from its intrinsic lean against the aggregates — the sizing spine's engine bounds annotating, never enforcing, the coherence checks below the one hard rail.
The step also emits the **action half of the what-changed audit**: a **changed action** attributed to a moved intrinsic verdict *or* a moved portfolio context; a weight-range move alone demands no attribution, since the range is rung-band-anchored to the current weight and so moves with pure weight drift.
And it emits the schema-validated **portfolio-level view** — concentration and sector/factor exposure, overall risk posture, a cash / deployment stance, and positions closed since the last run — read against both the house view and the profile.
The cash / deployment stance says what to trim to fund which adds, including **raising cash from a dead-money loser**, where the *possible* tax benefit of realizing the loss and the redeployment optionality of the proceeds are valid supporting rationale, framed high-level with the user acting on the specifics — and, under the unconstrained-cash profile, the **external funding the plan implies** ([portfolio-analysis.md §Portfolio roll-up and construction](portfolio-analysis.md#portfolio-roll-up-and-construction)).
The action-half attributions are **app-validated** the same way 6g validates the intrinsic half: a "became oversized" or "freed cash" claim must map to a real Step-7a aggregate, not a model assertion.
The deterministic solve then splits two ways under the v7 contract ([portfolio-analysis.md §Portfolio roll-up and construction](portfolio-analysis.md#portfolio-roll-up-and-construction)): **self-coherence stays enforced** — a sell-all's range must be 0–0, ranges must be well-ordered, and each stated range must contain its own jointly-implied weight (an incoherent plan gets the single named-violation re-run; persisting incoherence fails the run) — while every **engine-bound check annotates, never enforces**: a rung outside the engine set, a range outside its rung band, a concentration-cap breach, and unfunded buys each record on the construction view's `engine_bound_annotations` and render beside the plan, which persists exactly as authored.
The re-run is **repair-scoped** — corrected objects for the violating names only, merged over the first draft and re-validated whole (canonical at [portfolio-analysis.md §Portfolio roll-up and construction](portfolio-analysis.md#portfolio-roll-up-and-construction)).
A run failed at this stage still **persists its per-holding work**: the verdicts, audits, and Step-7a aggregates write as a **degraded run** — a run row with no constructed book — before the failure records, so a whole-book infeasibility no longer discards the completed pass (ruled 2026-08-11 — [verification/2026-08-10-big-run-attempt-1.md §Disposition](verification/2026-08-10-big-run-attempt-1.md)).
...
**Type:** Computed (persist the verdicts, roll-up, holdings snapshot, and audit record) + Local-model call (embeddings — as-built the matured outcome learnings alone; the per-holding verdict embeddings are designed).
...
The Portfolio-runs history shows **persisted work** rather than successes alone: a construction-failed run's degraded row appears marked as having no book ([§Step 7b](#step-7b-portfolio-construction)), while a cancel or any failure before Step 7b still leaves no row.

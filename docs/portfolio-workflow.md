...
**Type:** API retrieval (FMP / SEC EDGAR / Schwab / FINRA lookup) + Computed (assemble the packet); the local-model embedding call for semantic continuity retrieval is **designed, not built**.
...
**(b)** A conviction layer → the **momentum / market-setup read**, the **hard forensic state** (the hard-event kinds reading the shared typed producer — as-built the item-classified EDGAR filing kinds; the research-fed `forensic_event` fraud claim joins when the loop lands — [trade-opportunities-workflow.md §Step 5c](trade-opportunities-workflow.md#step-5c-deterministic-analysis-archetype-weighted-engine)), the **narrative-vs-reality ratio** (with its thin-coverage fallback — built), and the **implied-expectations read** (the shared Step-5c primitive — built); the soft **forensic flags** are designed, landing with their data leg ([portfolio-analysis.md §The per-holding pipeline](portfolio-analysis.md#the-per-holding-pipeline)) — all kept *out* of the letter.
...
That bypass is a stub-time shortcut, not a branch rule — when the loop lands a `role_risk_only` fund runs the fund agenda and a pure-consolidation distillation like any fund.
...
The engine-matched **cap rules persist as annotations that bind the engine arm**: a tripped hard forensic trigger (restatement / auditor change / fraud flag — the trip resolving only from the shared typed producer: an item-classified filing kind, or a validated `forensic_event` research claim from 6d, never a bare model assertion — [trade-opportunities-workflow.md §Step 5c](trade-opportunities-workflow.md#step-5c-deterministic-analysis-archetype-weighted-engine)), a repeated execution miss's Medium ceiling, severe deterioration's Low ceiling, and the narrative-vs-reality hype read's Medium ceiling each record on the audit and bound the **engine stand-in arm** — while the model's conviction and action persist as authored, the departure rendering beside the recorded rule ([portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable) stays the canonical hard-outcome record).
...
The repeated-execution-miss cap waits on the stubbed observation producer, and the hard-forensic **fraud** kind on the research-fed `forensic_event` claim, so neither fires yet.

...
The validated **what-changed audit** and the rewritten ledger are recorded against the prior run; output stays firm and does not swing run-to-run absent hard supporting data ([thesis-continuity.md](thesis-continuity.md)).
...
The Portfolio-runs history still lists **legacy degraded rows** from the retired construction era, each marked as having no book and opening read-only ([portfolio-analysis.md §Failure posture](portfolio-analysis.md#failure-posture)); a tunnel-vision run persists complete or leaves no row.

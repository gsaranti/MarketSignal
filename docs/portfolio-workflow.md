...
These typed fields exist only where their consumers do: a **`role_risk_only`** holding's distillation emits none — no target for an assumption to move, no conviction for an indicator to raise, and the hard outcome's legs all live on the priced branch — and is pure consolidation for interpretation.

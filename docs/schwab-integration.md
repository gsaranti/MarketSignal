...
Holdings are pulled from Schwab only on explicit user action — there is no scheduler, timer, or background sync. Two controls fetch, with different intents ([portfolio-analysis.md §Triggering](portfolio-analysis.md#triggering)): **Run analysis** always pulls fresh holdings as the job's first retrieval and analyzes them, and **Pull holdings** fetches and displays the current positions without analyzing (view-only; it needs no local-model configuration and never feeds the job or its diff baseline). Either way the user controls when fresh data is fetched — which also means the user controls when the periodic re-authentication (below) surfaces, rather than it appearing unexpectedly mid-session. The latest snapshot from each path persists — the run's with the run, the standalone pull's as the latest-holdings view — so the portfolio is viewable without re-fetching.

...
As-built the method is the **whole-chain read** — put/call by volume and by open interest over every contract returned, and skew as the chain-wide mean put IV minus mean call IV — with no greeks parsed, no liquidity floor or zero-bid exclusion, and no delta / moneyness banding.

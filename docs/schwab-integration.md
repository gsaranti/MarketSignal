...
Schwab's average cost is a **per-unit** `averagePrice`; the app derives each row's **cost basis as a signed account-currency total** (`averagePrice × signed quantity`), and every downstream mention of *cost basis* — display math, the roll-up, the action-sizing spine's unrealized P/L — means that derived total, never the per-unit price.

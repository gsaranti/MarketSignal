...
Each account's position rows yield the fields the analysis needs: instrument identity (symbol, CUSIP, asset type), quantity (one **signed net** value per account row — long minus short), average cost, market value, and profit/loss.

...
Chain timing follows **each job's own shape**: **Portfolio Analysis** fetches a chain **per holding at its Step-6 dossier assembly, fresh within the run** ([portfolio-workflow.md §Step 2](portfolio-workflow.md#step-2-load-holdings-and-fetch-option-chains) — so a selective run's carried tail spends no chain call), and **Trade Opportunities** fetches a chain **per surviving candidate at dossier assembly** ([trade-opportunities-workflow.md §Step 5b](trade-opportunities-workflow.md#step-5b-dossier-assembly) — candidates don't exist until discovery has run).

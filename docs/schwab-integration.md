...
Each account's position rows yield the fields the analysis needs: instrument identity (symbol, description, asset type — no CUSIP is mapped off the wire), quantity (one **signed net** value per account row — long minus short), average cost, and market value (profit/loss is derived downstream as market value − cost basis, not a pulled field); `currentBalances` supplies `cashBalance` and the account's authoritative `liquidationValue`.
Schwab's average cost is a **per-unit** `averagePrice`; the app derives each row's **cost basis as a signed account-currency total** (`averagePrice × signed quantity`), and every downstream mention of *cost basis* — display math, the roll-up, the action call's unrealized-P/L line — means that derived total, never the per-unit price.
The derivation applies **no option contract or bond par multiplier**, so for those two asset classes the derived total misstates the true basis — an option's understated, a bond's overstated — and the Portfolio card withholds every cost-derived figure rather than render a wrong number, pending the `averagePrice` multiplier probe ([portfolio-analysis.md §Storage and display](portfolio-analysis.md#storage-and-display)).
The payload carries **no bond analytics and no held-contract greeks** — a limit any future whole-book read inherits ([portfolio-analysis.md §Portfolio roll-up](portfolio-analysis.md#portfolio-roll-up)); a held option's greeks are readable only where a chain is fetched for its underlier (below).
...
A netted position whose summed quantity, cost basis, or market value does not finish as a finite number fails the pull naming the symbol, before any per-holding work is spent (ruled 2026-08-29 off the review round of the 2026-08-24 review's Codex I16).
The book's own sums — reconciled cash across accounts and the sum of reported account liquidation values (with a safe positions-plus-cash fallback only when no explicit cash row makes that fallback ambiguous) — fail the pull the same way (off I16's Codex round 1 and Review 2 M21).
...
The netting step is built and live at snapshot assembly — `Holdings::normalized()`, invoked on every run snapshot and on the standalone pull (alongside the full ticker→CIK resolver, [data-sources.md §SEC EDGAR](data-sources.md#sec-edgar)).
...
As-built the method is the **whole-chain read** — put/call by volume and by open interest over every contract returned, and skew as the chain-wide mean put IV minus mean call IV — with no liquidity floor or zero-bid exclusion, and no delta / moneyness banding.

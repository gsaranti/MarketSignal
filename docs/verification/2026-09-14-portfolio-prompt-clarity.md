...
- Interpretation, role/risk and every distillation shape show schema-derived examples with populated nested objects, array items, enums and nullable alternatives.
  Examples illustrate structure; their values prescribe no finding.
...
- Priced and role/risk action packets state only their applicable decision basis.
  ...
  The letter has no fixed model-action mapping, and the engine's selected action remains undisclosed.
...
Final checks passed: `cargo test -- --quiet` (1,444 unit tests and 32 integration tests passed; 31 ignored), `cargo clippy --all-targets --all-features` without warnings, `npm run build`, and `npm test` (46 pure-module tests and 255 component tests).

...
I20 is resolved (2026-08-29, its own slice; the rulings and resolution are recorded under §I20, the trail's shape stamped `checkpoint-v3`), leaving nothing — every finding the record raised is handled.
...
Ruled 2026-08-29 (at its plan): the row's new shape moves the trail's format stamp — `store::CHECKPOINT_FORMAT_VERSION` to `checkpoint-v3` — on §I18's contract, a field the checkpoint row carries changing the trail's shape and nothing a completed holding's verdict or audit means.
The admission filter, the 6d schema, the prompt lines and the overlay's computation are unchanged, and the stamp is app-written, never model-authored.
Ruled 2026-08-29: `portfolio::PROMPT_VERSION` stays `portfolio-v23` and `PRE_PROFIT_PARAMETER_VERSION` stays `pre-profit-v3`.
I3's `portfolio-v17` moved the prompt stamp because the 6d schema and a prompt line changed, neither of which I20 touches, so a `portfolio-v24` would put a row stating no prompt content changed on the prompt axis.
The debut run's rows therefore all stamp `portfolio-v23`.
Ruled 2026-08-29: the model-facing row and the persisted row are two types — `ObservationCandidate`, the eleven model fields (the 6d wire row and the pre-admission candidate), and `PreProfitObservation`, the candidate plus `admitted_under: String`, which `validate_observations` writes on acceptance as `portfolio::PROMPT_VERSION`.
A rejected row persists as the candidate, taking no stamp — the audit's own `prompt_version` names the contract that rejected it.
Ruled 2026-08-29: the stamp stays outside the dedup key, so a stored row re-offered under a later contract rejects as a duplicate and keeps its first stamp — the shape "never re-admitted through a later filter" entails.
Ruled 2026-08-29: the stamp renders nowhere this slice — rows render in no prompt and the frontend carries no observation type — persisting as attribution alone, for the audit and a later calibration slice's read.
Ruled 2026-08-29: the contract is canonical at `portfolio-workflow.md` §Step 6e, the admission contract's home since I3, with one-line mirrors at `storage.md`'s persisted-row sentence, `portfolio-analysis.md` §Starting parameters beside the overlay's parameter-version sentence, the logic-flow's 6e validation and persistence bullets, and the watch set's slice list and producer-activation line.
**Resolved 2026-08-29.**
As-built before the fix, `PreProfitObservation` was at once the 6d wire row, the pre-admission candidate and the persisted history row, and carried no field naming the contract that admitted it.
The fix splits the model-facing row from the admitted one: `ObservationCandidate` carries the eleven model fields (the 6d wire type in `distill.rs`, and the candidate `validate_observations` judges), and `PreProfitObservation` carries the same fields plus `admitted_under`, written by `ObservationCandidate::admit` at acceptance as `portfolio::PROMPT_VERSION` — the one place an admitted row is built from a model row, so the model can never author the stamp and no admitted row lacks it.
`RejectedObservation` holds the candidate, so a rejected row takes no stamp: the list is rebuilt from the candidate batch on each re-analysis and never read from the prior overlay, and a carried verdict carries its prior audit whole, rejected rows included, the audit's own prompt version naming the contract either way.
The dedup key (`dedup_key_of`, shared by both types) leaves the stamp out, so a stored row re-offered under a later contract rejects as a duplicate and keeps its first stamp; `merge_observations` is unchanged and the history carries whole.
The field has no serde default; the 6d schema and prompt line are unchanged, and the schema is pinned to omit the stamp from the model's row.
The trail's shape stamp moves to `checkpoint-v3` with its history clause, so a `checkpoint-v2` trail — its overlay rows unstamped — refuses at the resume gate with the format reason; no other axis moves.
Six tests pinned (two added, four re-based): an accepted row stamped with the prompt version at acceptance and its rejected twin returned as the candidate; a history row admitted under `portfolio-v17` riding the merge with its own stamp beside a fresh acceptance's, the same fact re-offered rejecting as a duplicate with the first admission standing; the overlay round-trip carrying the stamp and refusing to decode a row without it; the run round-trip carrying an accepted row's own stamp through the store; the resume gate refusing a `checkpoint-v2` header beside the `v1` case; and the 6d schema omitting the stamp.
Canonical at `portfolio-workflow.md` §Step 6e (the stamp, the never-re-admitted rule, the dedup rule, the rejected row), mirrored at `storage.md`'s persisted-row sentence, `portfolio-analysis.md` §Starting parameters beside the overlay's parameter-version sentence, the logic-flow's 6e validation and persistence bullets, and the watch set's slice list and producer-activation line.
One reviewer round (2026-08-29): approve-with-nits, every criterion passing and the scope report judged honest.
The five sentence-per-line nits are folded in — the §Step 6e sentences split at their standalone claims, the watch set's slice recorded as its own sentence rather than a trailing clause, and its stamp line split with "first working read" reworded to "first attributable read", nothing reading the stamp this slice.
The `dedup_key_of` eight-parameter note is declined: the explicit-duplication shape is the ruled one (no `Deref`), and a borrowed-view struct returns only if the key grows.
Codex round 1 (2026-08-29): approve with one low finding — "rebuilt each run and never carried" overclaimed, since a selective run's unselected holding carries its prior audit whole (`job.rs`, the carried-verdicts block), the overlay's rejected list included, under the prior audit's own `prompt_version`, so attribution held while the lifecycle wording did not.
Closed as wording at the `RejectedObservation` doc, §Step 6e, the logic-flow's persistence bullet (whose pre-existing "not carried" clause carried the same imprecision) and this section's ruled line and resolution: a rejected row is rebuilt from the candidate batch on each re-analysis and never read from the prior overlay, a carried verdict carrying its prior audit whole.
No code moves and no stamp moves off the round.
Codex round 2 (2026-08-29) found the lifecycle finding resolved, with one editorial nit — the §Step 6e sentence's trailing absolute ("the prior overlay's rejected rows never read") — folded in as a full clause; no code moves and no stamp moves off the round.
Codex round 3 (2026-08-29) approved with no remaining findings.

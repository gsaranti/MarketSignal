...
## Finding 3 — fix (landed 2026-09-01)

The per-holding action call's prompt friction is fixed by the post-run candidate this record diagnosed: the reasoning-guidance layer is cut and the behavioral contracts are kept, stated once as a short ordered list rather than woven into a how-to-weigh narrative and no longer duplicated across the system and user prompts.
`action_system_prompt` is rewritten as data framing plus an ordered gate list — the verdict is given below as the evidence, the gates state what each input is in the order it bears on the rung (grade and both arms' target moves first, then risk tier and horizon, the investor profile as the tie-breaker, tax as a required flag, capital efficiency as an exit input only on a `fails` read, the engine set as evidence, and the role/risk branch) — and the enumeration that had told the model to weigh grades, scores, conviction, horizon, and implied moves is dropped, since a frontier reasoner does that unasked.
`action_user_prompt` is reduced to values: the tax read renders the sign and its possible benefit/cost without the "flag, never the mover" rule, the engine-arm block renders the capital-efficiency read without its neutrality rule, the target-provenance line drops the "weigh by it" instruction, and the engine-set block renders the set without repeating the departure contract — each rule now living once in the system prompt.
On a user follow-up the scope framing was cut further: the standalone "tunnel vision is the contract" paragraph — which narrated the app's whole-book architecture (a separate planning stage, the out-of-scope cash/sector/concentration inputs, "no unseen portfolio context to hedge against") to a model that can neither see nor use any of it — is replaced by a single clause scoping the task to "its verdict … and the investor profile alone".
Tunnel vision is enforced by input isolation — the user prompt renders none of the whole-book fields — so the prompt no longer explains the design decision to the model, only the task; a test pins that the whole-book vocabulary is absent from the system prompt.

Two user rulings shaped it, recorded via the selector UI on 2026-09-01.
First, the posture: each behavioral contract is recast from a prohibition into a fact about what the input means or what is out of scope — capital efficiency "is an exit input only on a `fails` hurdle … a `clears` or `indeterminate` read carries no exit signal" rather than "do not let it tilt toward selling"; tax framed as "account type, lots, and rates are unmodeled, so [it] is a user-facing flag" rather than "never the mover" — so the model draws the inference itself, which keeps every contract's enforcement (attempt 3 regressed without them) while discharging the prompt-posture invariant this prompt was the named violation of.
Second, the scope: the shared `pre_profit_prompt_section` Action-stage consequence lines are de-prescribed in the same pass, dropping the "weigh the deterioration/financing evidence before departing" nudges for the engine-rule fact and the "your rung is UNRESTRICTED" two-arm fact, and the Interpretation-stage branches are aligned to match so the one helper keeps a single posture.

This is a prompt-clarity and throughput fix, not a correctness fix: on TSLA the oscillating trace still reached the correct persisted verdict, because the thinking block is a free side-channel the grammar and validators sit downstream of.
So whether the collapse from branch-by-branch self-checking to a short ordered deduction actually cuts the per-holding wall clock is attempt 5's to measure — the quantify-across-the-book action stands: note, per holding, whether the action trace oscillates across rungs before settling.
`PROMPT_VERSION` stays `portfolio-v34`, since v34 has never run and every pre-debut change folds into the debut stamp.
Verification: `cargo test` 1,431 library tests passed with 31 live smokes ignored (plus the integration suites), warning-free `cargo clippy --all-targets --all-features`, frontend untouched; the wording-pinning tests were updated to assert the contracts appear once in the system prompt as facts and are absent from the user prompt.

### Full portfolio-prompt posture sweep (2026-09-01)

The Finding-3 fix prompted a user request to audit every Portfolio Analysis prompt against the same lens and the `docs/local-models.md §Prompt posture` principle: no app-architecture narration, no author's financial-analysis biases, no unneeded instructions, no explaining-how-to-think.
The surface swept was the interpretation system and user prompts, the role/risk system and user prompts, the ~15 evidence sections, the ledger authoring block, and the response contracts (`pipeline.rs`), plus the research and distillation prompts (`research.rs`, `distill.rs`).
The evidence sections and the ledger and distillation contracts were overwhelmingly clean — raw data plus load-bearing contracts the schema cannot enforce — so the leakage was concentrated in the connective prose, in the same two failure modes Finding 3 named.

The architecture-narration cuts removed pipeline mechanics the model can neither see nor act on: the interpretation and role/risk prompts' "a dedicated decision stage sets the action afterward", the "both arms are scored by a deterministic scoreboard" and "the scored comparison is the deterministic scoreboard's" references, the model-arm header's "scored against realized outcomes beside the engine baseline", the role/risk `ACTION` block's downstream engine-set / departure-annotation mechanics, the interpretation prompt's "the profile enters at the action decision only", and the research system prompt's "a separate step writes the structured findings" — each replaced by the task or the fact alone.
The how-to-think cuts removed instructions a frontier reasoner does not need once handed the fact: "let it discipline this run's numbers" and its retrospective twin, the forensic section's "weigh the event's severity and recency" and "weigh accordingly", the implied-expectations "read your forward outlook against this … not a vibe" editorializing, the option-overlay "this changes what the right action is", and the technology-event flag's "weigh whether a real technology event explains the move before treating it as impairment or opportunity" — keeping in each case the engine-rule fact, the flag, or the datum.

The one judgment call was the `TARGET PROVENANCE` weighing block and its duplicate in the capital-efficiency line, which encode the 2026-07-31 F6 failure (a sell-all cascade that over-trusted low-signal targets).
The user ruled trim-to-facts: the multi-sentence "a `fails` that survives even the widened bull leg is robust exit evidence; over the low-signal carry shape … a `fails` there stays weak exit evidence" narrative and the capital-efficiency "one input to weigh … weak evidence for an exit, not an instruction" cross-check are cut, while the provenance-type facts they rested on — "the current multiple was carried, so the targets hug the current price and carry little forward signal", "spread-anchored on N rate observations", the flat-driver and dispersion-floor flags — remain rendered beside the read, so the model infers the signal-quality weighting itself.
Attempt 5 re-checks the regression risk the F6 lesson guarded against.

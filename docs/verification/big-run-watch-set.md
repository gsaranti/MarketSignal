...
Records this run persists stamp `portfolio-v35`, not the `portfolio-v11` these additions were first queued under — the run-evidence, infrastructure, research-loop, F3 tie-channel, expense-ratio render, ledger-basis vocabulary, IV-skew convention, observation-excerpt, guidance-vintage, action-target, model-arm-domain, finite-record (Codex I7 / I9 / I16), prompt-render / period-word-guard (Codex I8 / I10 / I12 / I19), continuity-attribution (Codex I11 / I13), Review 2 period-span, constant-period revision, assumption-unit, fund-classification, industry-routed commodity-context, comparison-safe delta, action-provenance, profile-tax, typed distillation-channel, Finding-2 schema-carrying prompt-clarity, Finding-3 action-call prompt-clarity, Finding-4 Fix-B split plus post-landing context/evidence hardening, and Finding-5 synthesis-shape prompt-clarity slices all landed since the 2026-08-18 revision, and the fresh-start-2 compat cut landed with no stamp.
...
The industry-routed commodity context: every uranium-industry holding receives the dated uranium print and no WTI / Henry Hub line, while every coal-industry holding receives no oil / gas proxy.
For any carried stock stopped by the listing guard, confirm no sector-benchmark fetch or benchmark-gap telemetry survives the terminal route.
...
The per-holding budgets, calibrating on this evidence: the 40-attempt fetch ceiling (`MAX_FETCHES_PER_HOLDING`; failed live attempts spend it), the 30-minute wall clock (`MAX_WALL_PER_HOLDING`), the 4,000-char per-topic seed budget (`SEED_BUDGET_CHARS`), and the pass-shape constants (`MAX_TURNS_PER_PASS`, `MAX_TOOL_CALLS_PER_TURN` = 8, `MAX_PASSES_PER_TOPIC`, `PAGE_TEXT_CAP_CHARS`, `MAX_CLAIMS_PER_PASS`, `MAX_SEEDED_BY_PER_PASS` = 4 distinct known ids) — all kept as drafted until this run reads them (ruled 2026-08-27, with the Fix-B batch/input bounds added before its debut).
...
Every accepted row persists `admitted_under` = `portfolio-v35` (Codex I20 plus Review 2 N1; later revision, assumption-unit, fund-classification, commodity-context, prompt-contract, distillation-channel, Finding-2 prompt-clarity, Finding-3 action-call prompt-clarity, Finding-4 Fix-B, and Finding-5 synthesis-shape slices moved the shared prompt stamp).
...
The Finding-3 action-call prompt clarity (introduced under `portfolio-v32`, attempted under current `portfolio-v35`): confirm the model emits only the rung and one-sentence rationale with the departure app-stamped on any outside-set choice, and that a `clears` or `indeterminate` capital-efficiency read leaves the rung untilted toward selling — the soft sell-lean the finding flagged on the two-holding sample.

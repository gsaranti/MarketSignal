...
Every accepted row persists `admitted_under` = `portfolio-v31` (Codex I20 plus Review 2 N1; later revision, assumption-unit, fund-classification, commodity-context, prompt-contract, distillation-channel, and Finding-2 prompt-clarity slices moved the shared prompt stamp).

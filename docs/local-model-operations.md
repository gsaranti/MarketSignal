...
- **Eval-stats caveat for any instrumentation** *(2026-07-31)*: on v0.32.5 the `/api/chat` terminal-chunk `eval_count` / `eval_duration` cover only the post-thinking content phase — the thinking phase rides inside `total_duration` unaccounted.
  Characters divided by app elapsed seconds yield characters/second, not tokens/second; no tokenizer-free conversion is reported as an exact token rate.
  Record app elapsed time, decoded thinking characters, and the six raw API counters separately, using the measurement definitions in [local-models.md §The local-model adapter seam](local-models.md#the-local-model-adapter-seam).
  The fence labels the runtime-returned `eval_count` directly; it never presents that count as thinking plus content.

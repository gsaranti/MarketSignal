...
**Embedding responses are validated, not trusted** — one shared validator behind the trait, applied to every embedding call, local and cloud alike: the response must carry **exactly one vector per input**, from the **configured embedder identity**, with **every element finite** and a **nonzero norm**, over a **byte-capped input** — anything else is rejected and logged, never stored or searched.
Two of those are poisoning guards rather than hygiene: a non-finite component makes every cosine comparison it touches `NaN`, and a zero vector has no direction, so its cosine is undefined — both corrupt recall silently, for the life of the row, instead of failing.
A wrong-identity response is a roster misconfiguration surfaced, not a vector: a different embedder writes into a different space, which cosine cannot detect.
The identity check is deliberately tolerant — case- and whitespace-insensitive, accepting either name as a prefix of the other so a quantization or `:latest` tag is not a false failure, and skipped entirely where the provider echoes no model.
The **input byte cap** is applied in the request builders, so it binds every call rather than only the ones whose caller remembered: bytes rather than tokens or chars because `tokens ≤ bytes` is what makes the provider's limit provable without a tokenizer dependency.

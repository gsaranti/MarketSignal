...
The classification is a whitelist: a deadline trip, a length stop, a cancelled run, and any unclassified failure never enter the general retry — the first two are attributable or deterministic, so repeating the same request would spend hours reproducing a known outcome.

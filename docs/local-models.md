...
A *configuration-blocked* local job surfaces in the Persistent Warning Area under its own presence-based categories (local models not configured, Schwab connection — distinct from the report's) or, for the FMP / FRED keys, the shared **missing provider credentials** category.

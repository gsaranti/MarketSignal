...
**Presence** of the config values — the Ollama endpoint and the roster ids (the 122B reasoner + the embedder; the fast tier is optional) — is known synchronously and gates *proactively*: if any is unset, the local-suite Run buttons are locked and a persistent **local models not configured** warning shows, cleared the instant the values are filled (exactly like the cloud model selectors / data-source tokens).
...
The local embedder implements the **same `Embedder` trait** the report pipeline already defines, so vector-memory storage and retrieval are reused unchanged (only the vector space differs — see below).
**Embedding responses are validated, not trusted** — one shared validator behind the trait, applied to every local embedding call: the response must carry **exactly one vector per input**, from the **configured embedder identity**, at the **configured dimensionality**, with **every element finite** and a **nonzero norm**, over a **byte-capped input** — anything else is rejected and logged, never stored or searched (a zero-norm or non-finite vector would poison every cosine comparison it touches; a wrong-identity response is a roster misconfiguration surfaced, not a vector).
...
- **Compute, don't guess — and since `portfolio-v7`, compute the baseline.**
  ...
  In **Portfolio Analysis** the engine's values are the **baseline arm** of a two-arm verdict: the model additionally authors its **own** clearly-typed arm of the judgment fields (sub-scores, target bands, conviction, outlook, lean, action), unrestricted and scored against the baseline by a deterministic scoreboard — but engine-owned values are **app-stamped directly, never echoed through the model**, and the model arm's judgment values never alter or bind the engine baseline (the boundary statement, with its intentional downstream consumers and the typed validated channels, at [portfolio-analysis.md §The holding verdict](portfolio-analysis.md#the-holding-verdict)) — so a missing input still becomes a gap, never a fabricated level, on the arm the machinery trusts.
  **Trade Opportunities keeps the original single-arm rule as designed**: the model interprets computed values and never invents a number, with an engine-owned number in a model return schema either a **selection identifier** the app resolves to its own value or a **numeric echo validated for exact equality** — a mismatch rejecting the response ([trade-opportunities-workflow.md §Step 5h](trade-opportunities-workflow.md#step-5h-deterministic-risk-tier-gate-validation--checkpoint)).
...
A job's learnings are specific to its work (holding-grading calibration is not opportunity-discovery context, and neither is market-thesis memory), so cross-job recall would be noise.
This isolation governs *learnings only*: the Market Signal Report remains available to the local jobs as a **read-only shared input**, but it enters deterministically (see [§Context-memory discipline](#context-memory-discipline)) — never by searching the report's vector partition.
Isolation is enforced by an explicit **job namespace** on each memory row — orthogonal to the existing summary/learning entry kind, which inside a local namespace is the **purpose boundary** (continuity summaries vs calibration learnings; Trade Opportunities rows additionally lifecycle-tagged — [storage.md §Local Vector Memory](storage.md#local-vector-memory)) — with every retrieval scoped to the calling job's namespace, because the two local jobs share an embedder and dimensionality and so are not separated by dimensionality alone (the report is additionally separated structurally: it embeds with OpenAI `text-embedding-3-large`, a different dimensionality from the local embedder).
All three reuse the same `vector_memory` module and `Embedder` trait; they differ only in which partition they touch (see [storage.md](storage.md)).
...
The local-job execution gate — the daemon + roster **configured**, **a connected Schwab account** (required by both jobs, since holdings and the options-activity signal come from Schwab), the shared **FMP / FRED credentials present** ([portfolio-workflow.md §Step 1](portfolio-workflow.md#step-1-job-start-and-gate)), and the daemon actually **reachable** for any run that calls the model — is the precondition that blocks a run, and it is independent of the cloud-report gate; a *configuration-blocked* local job surfaces in the Persistent Warning Area under its own presence-based categories (local models not configured, Schwab connection — distinct from the report's) or, for the FMP / FRED keys, the shared **missing provider credentials** category, while a live connectivity failure caught at the run-gate blocks the attempt inline rather than as a persistent warning — except the engine-only paths that make no model call — ATO's **Quick Audit** and Portfolio's **Quick check** — which skip the daemon-connectivity check, running even with the daemon configured-but-down ([trade-opportunities.md §Failure posture](trade-opportunities.md#failure-posture), [portfolio-analysis.md §The quick check](portfolio-analysis.md#the-quick-check-engine-only)); a local job that *fails mid-run* surfaces under the shared **Failed jobs** category, like a failed report ([interface.md §Connection status](interface.md#connection-status-local-suite)).

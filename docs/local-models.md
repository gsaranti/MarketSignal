...
- **Compute, don't guess.**
  ...
  This keeps the suite faithful to the app's data-honesty stance: a missing input becomes a gap, never a fabricated level.
  The boundary is **enforced by the app, not the prompt**: an engine-owned number appearing in a model return schema is either a **selection identifier** the app resolves to its own value, or a **numeric echo validated for exact equality** (set membership, where the model selects among engine scenarios) against the engine's record — a mismatch **rejects the response** like any schema failure, the same model-proposes / app-validates spine as the conviction raise.
  Both verdict-writing validators apply the rule ([portfolio-workflow.md §Step 6g](portfolio-workflow.md#step-6g-continuity-check-and-checkpoint), [trade-opportunities-workflow.md §Step 5h](trade-opportunities-workflow.md#step-5h-deterministic-risk-tier-gate-validation--checkpoint)).

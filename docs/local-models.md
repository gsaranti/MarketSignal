...
The suite's stages chain through **distilled, schema-shaped hand-offs**, never raw transcripts. Each stage emits a compact validated object; the next stage receives only that object plus the specific evidence it needs — not the prior model's full output. Three rules enforce this:
...
- **Retrieve, don't dump.** Supporting context from prior Market Signal Reports and from earlier runs enters a stage through **vector retrieval of the relevant slice**, not by stuffing every recent report into the prompt.
- **Forward only what's needed.** A research stage's output is condensed (by the fast model) into a findings object before the analysis stage sees it, so analysis reasons over evidence, not over the research transcript.
...
**Each job's memory is isolated to that job.** There are three independent memory partitions — the Market Signal Report, Portfolio Analysis, and Trade Opportunities — and a job retrieves only from its own. A job's learnings are specific to its work (holding-grading calibration is not opportunity-discovery context, and neither is market-thesis memory), so cross-job recall would be noise. The report is also separated *structurally*: it embeds with OpenAI `text-embedding-3-large`, while both local jobs embed with the same local model, so report vectors are a different dimensionality and cannot be compared against the local ones. The two local jobs **do** share an embedder and dimensionality, so their isolation is enforced by **partition** — each job writes and reads only its own memory kind — not by dimensionality. All three reuse the same `vector_memory` module and `Embedder` trait; they differ only in which partition they touch (see [storage.md](storage.md)).
...
The research half of each local job is **fail-soft**: a flaky web search degrades the evidence for an item rather than failing the run. The local-job execution gate (daemon reachable, roster present, and — for portfolio — holdings available) is the one precondition that blocks a run, and it is independent of the cloud-report gate. Cancellation is cooperative through the shared `progress` seam, identical to a report run.

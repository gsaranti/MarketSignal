...
Isolation is enforced by an explicit **job namespace** on each memory row — orthogonal to the existing summary/learning entry kind — with every retrieval scoped to the calling job's namespace, because the two local jobs share an embedder and dimensionality and so are not separated by dimensionality alone (the report is additionally separated structurally: it embeds with OpenAI `text-embedding-3-large`, a different dimensionality from the local embedder).

...
Each holding card carries a **selection control** (with select-all / deselect-all) that turns **Run analysis** into a **selective re-analysis** over the selected holdings **plus the automatic safety inclusions**: new holdings, quick-check flags, `unknown` degraded-sweep results, side reversals, unexamined evidence events, and over-age exit-family carries; an empty selection runs the full book ([portfolio-analysis.md §Triggering](portfolio-analysis.md#triggering)).

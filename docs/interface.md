...
A **legacy degraded run** — the retired construction era's failed-construction shape, verdicts persisted with no book ([portfolio-analysis.md §Failure posture](portfolio-analysis.md#failure-posture)) — lists like any other row with a quiet **no book** tag.
A degraded run's read-only view banners that the actions shown are per-holding reads rather than a constructed plan.
A role/risk holding's action is suppressed entirely on that view: under the retired era's contract construction alone authored it, so the persisted value is a placeholder, never a decision.
...
Selecting the **newest constructed** run row shows the live Portfolio page; selecting **any other** — an older run, or a degraded newest — renders that persisted run on the Portfolio page **read-only** — an informational banner names the run's vintage and carries a *Back to latest* control, the Run analysis / Pull holdings triggers lock with the reason stated, and the current-holdings comparison section (whose churn tags are keyed to the *latest* vintage) never renders over a past run.
A run row appears only when the run **persisted work** — on success, or as one of the retired construction era's legacy degraded rows ([portfolio-analysis.md §Failure posture](portfolio-analysis.md#failure-posture)) — so a cancel or a mid-run failure adds nothing, and the run-is-never-a-report invariant ([run-tracking.md](run-tracking.md)) is unchanged for reports.
...
Each holding card carries a **selection control** (with select-all / deselect-all) that turns **Run analysis** into a **selective re-analysis** over **strictly the selected holdings** (ruled 2026-08-16, [verification/2026-08-16-selective-badges-ruling.md](verification/2026-08-16-selective-badges-ruling.md)); the rest carry forward, and the quick check's findings — attention flags, `unknown` degraded-sweep notes, unexamined evidence events, side reversals, and stale vintages — ride the cards as **non-blocking badges** rather than forcing a re-analysis. A held position with no prior verdict to carry is left **not analyzed**, shown as a selectable *run to grade* placeholder card; an empty selection runs the full book ([portfolio-analysis.md §Triggering](portfolio-analysis.md#triggering)).
...
Since both suite jobs went two-arm the card carries the engine baseline too, but **by progressive disclosure rather than Portfolio's paired columns**: a TO cell holds several cards and the 3×3 is meant to be scanned, so the matrix card shows the model arm's conviction and outlook with a quiet **divergence tag** in the existing badge family where the arms materially disagree (conviction level, band overlap, or a recorded advisory tier / horizon divergence), the paired engine / model view appears **on card expand** under the measured-reveal idiom, and the **List view** carries the pairing natively as engine-target / model-target / divergence columns in the `.ana-grid`, where comparison is the task.
...
The Trade Opportunities page also carries a **Matrix / List view toggle**: the 3×3 matrix is the default and canonical view, and List flattens all nine cells into one sortable ranking (default sort: forward % upside to the **model arm's** target, matching the card's headline, with the engine's a sortable column of its own; realized since-flagged return offered as the alternate key), each row keeping its risk tier, horizon, per-card selection control, lifecycle badges, and the two-arm target / divergence columns — a display-only reordering, detailed in [trade-opportunities.md §Storage and display](trade-opportunities.md#storage-and-display).

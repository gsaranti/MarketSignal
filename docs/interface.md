...
│   ├── Portfolio runs · last 30 (Portfolio view; an older run opens read-only)
...
│   ├── Holdings (Pull-holdings view-only refresh; Run analysis always re-pulls / CSV import supplement — designed, not built)
...
The Trade Opportunities view, when that feature ships, swaps in its own retained runs the same way — one `trade_opportunities` pool, each row labeled with its mode (`discover` / `audit-quick` / `audit-deep`; ruled 2026-08-19).
...
The **footer's job readout is section-scoped**: it reports the active section's own job — the report job everywhere except the Portfolio view, which reports Portfolio Analysis, and the Trade Opportunities view, which reports the one `trade_opportunities` job, every run of either trigger stamping the footer with its mode label (ruled 2026-08-19) — the same per-feature mapping as the history swap; the **"Generate now" trigger renders only on the report view** — each section keeps its own trigger (*Run analysis* on the Portfolio page; the Trade Opportunities controls with that feature).
...
Selecting the **newest readable** run row shows the live Portfolio page; selecting **any other** — an older run — renders that persisted run on the Portfolio page **read-only** — an informational banner names the run's vintage and carries a *Back to latest* control, the Run analysis / Pull holdings triggers lock with the reason stated, and the current-holdings comparison section (whose churn tags are keyed to the *latest* vintage) never renders over a past run.
A run row appears only when the run **persisted on success** — so a cancel or a mid-run failure adds nothing (a corrupt row still lists as unreadable), and the run-is-never-a-report invariant ([run-tracking.md](run-tracking.md)) is unchanged for reports.
A fresh successful run closes any open past-run view, and a run pruned by the 30-run retention leaves the list — an open past run that falls out of retention closes back to the latest view.
...
Live **connectivity** failures are *not* persistent warnings: a local-model failure (daemon unreachable, a model not pulled) surfaces at the run-gate as an inline block on the run attempt (the engine-only paths excepted — ATO's Quick Audit and Portfolio's Quick check make no model call, so they skip the daemon-connectivity check), while a Schwab *API* outage surfaces at the first Schwab fetch the job makes, not at the run-gate — Portfolio's Step-2 holdings load fails that run, while Trade Opportunities degrades a candidate's option-chain fetch to a typed gap and fail-softs its closing holdings cross-reference ([schwab-integration.md §Failure posture](schwab-integration.md#failure-posture), [§Connection status](#connection-status-local-suite)).
...
The Portfolio page must present each holding's **intrinsic verdict and portfolio action as distinct but linked**, with the action call's rationale visible (see [portfolio-analysis.md §The holding verdict](portfolio-analysis.md#the-holding-verdict)), so a deliberate pairing like *A-grade / trim* — a dead-money read or a profile tilt, never book context — reads as intentional rather than contradictory.
As-built (the tunnel-vision contract, `portfolio-v9`) each card shows the **rung-only action with the action call's one-line rationale** — no target-weight band, share/dollar figure, lean tag, or action-half audit line, all retired with the construction stage; the roll-up card carries the deterministic counts and reads plus the positions closed since the last run ([portfolio-analysis.md §Portfolio roll-up](portfolio-analysis.md#portfolio-roll-up)).
...
Not-analyzed placeholder cards sort in the same order on the same position-level keys (ruled 2026-08-18).
The roll-up card carries the run's one-line **data-health readout** (how the target surface was sourced — anchored vs fallback — plus deep-history and rate-history degradations, from the persisted aggregate: [portfolio-analysis.md §Portfolio roll-up](portfolio-analysis.md#portfolio-roll-up)); its **attention state reuses the sanctioned amber treatment** (`--grade-d-tx` on `--grade-d-bg`), and a run persisted before the aggregate existed simply shows no readout.

Like the Trade Opportunities page, the Portfolio page is **selection-aware and badge-bearing**.
Each holding card carries a **selection control** (with select-all / deselect-all) that turns **Run analysis** into a **selective re-analysis** over **strictly the selected holdings** (ruled 2026-08-16, [verification/2026-08-16-selective-badges-ruling.md](verification/2026-08-16-selective-badges-ruling.md)); the rest carry forward and ride the cards as **non-blocking badges** rather than forcing a re-analysis: the quick check's findings — attention flags, `unknown` degraded-sweep notes, and unexamined evidence events — plus the carried verdict's own **side reversals** and **stale vintages**, these last read from persisted verdict state rather than the sweep.
A held position with no prior verdict to carry is left **not analyzed**, shown as a selectable *run to grade* placeholder card; an empty selection runs the full book ([portfolio-analysis.md §Triggering](portfolio-analysis.md#triggering)).
A holding whose fresh analysis **failed** this run carries an **analysis-failed badge** ([portfolio-analysis.md §Failure posture](portfolio-analysis.md#failure-posture)): its prior verdict shows carried and vintage-stamped beside the badge where one exists, or the *run to grade* placeholder becomes an **empty failed card** naming the cause where none does — either way selectable, so a re-run targets just it, and the roll-up's key-figure strip carries a **Failed** count.
An engine-only **Quick check** control re-evaluates the standing ledgers between runs ([portfolio-analysis.md §The quick check](portfolio-analysis.md#the-quick-check-engine-only)).
An amber, *actionable* **attention flag** — the same treatment as Trade Opportunities' *Consider Deep Audit* (**"amber" names the state, not a token**: the actionable-warning badges render from the existing **`--grade-d-tx` on `--grade-d-bg`** pair, AA on its tint, noted in `colors_and_type.css` — no new color tokens) — rides a card the quick check has flagged on one of its four deterministic triggers ([portfolio-analysis.md §The quick check](portfolio-analysis.md#the-quick-check-engine-only)).
The next successful full pass over that holding consumes the trigger in interpretation / continuity, clears and acknowledges the flag rather than re-emitting it.
A quiet, *informational* badge in the **Research-stale family** marks an over-age verdict, an **unexamined evidence event** with no full pass since (the canonical evidence-event list — [portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable)), or a **degraded sweep** (the signal families the quick check couldn't verify — [portfolio-analysis.md §The quick check](portfolio-analysis.md#the-quick-check-engine-only)) — never the amber action color, mirroring Trade Opportunities' amber-is-a-problem discipline.
After a selective run each **carried card** carries its **analysis-vintage stamp** (a current-vintage card is unstamped, reading as the run's own date on the roll-up header), so mixed-vintage verdicts read as labeled vintages rather than a blend ([portfolio-analysis.md §Storage and display](portfolio-analysis.md#storage-and-display), [§The quick check](portfolio-analysis.md#the-quick-check-engine-only)).

The **Trade Opportunities** matrix card likewise surfaces each idea's **price prediction as a first-class, user-facing element** — the **model arm's** base-case target with its bear / bull range over the fixed twelve-month target window (rolling from the run date), its computation methodology accessible (an honest projection with a range, not a guarantee) — shown alongside the directional thesis, conviction, leading metric, catalyst, and (for a carried-forward idea) the since-flagged performance, so the **predicted target and the realized path sit together** ([trade-opportunities.md §The opportunity](trade-opportunities.md#the-opportunity), [§Storage and display](trade-opportunities.md#storage-and-display)).
Since both suite jobs went two-arm the card carries the engine baseline too, but **by progressive disclosure rather than Portfolio's paired columns**: a TO cell holds several cards and the 3×3 is meant to be scanned, so the matrix card shows the model arm's conviction and outlook with a quiet **divergence tag** in the existing badge family where the arms materially disagree (conviction level, band overlap, or a tier / horizon divergence — the engine's derived pair against the model's placement), the paired engine / model view appears **on card expand** under the measured-reveal idiom, and the **List view** carries the pairing natively as engine-target / model-target / divergence columns in the `.ana-grid`, where comparison is the task.
Since the placement ruling (2026-08-19) the matrix cell itself is the **model arm's** authored tier × horizon, with the engine's rule-derived pair among the expand view's paired values ([trade-opportunities.md §The opportunity space](trade-opportunities.md#the-opportunity-space)).
One honesty requirement rides the card: matrix membership is an **either-arm gate fact** while the headline number is the model's, so **both single-arm admission states are tagged**, with `engine-only` the more misleading one — there the displayed model target is the one that *failed* the gate and the engine's is what admitted the name, while `model-only` says the opposite (the headline admitted it, the deterministic baseline dissented); consensus cards carry no tag ([trade-opportunities.md §The opportunity](trade-opportunities.md#the-opportunity), [§Storage and display](trade-opportunities.md#storage-and-display)).
The divergence tag reuses Portfolio's tag treatment rather than introducing a component; the paired-comparison grid stays Portfolio's recorded design-system extension and is not re-cut for this surface.
...
The Trade Opportunities page also carries a **Matrix / List view toggle**: the 3×3 matrix is the default and canonical view, and List flattens all nine cells into one sortable ranking (default sort: forward % upside to the **model arm's** target, matching the card's headline, with the engine's a sortable column of its own; realized since-flagged return offered as the alternate key), each row keeping its placed model-arm risk tier and horizon, per-card selection control, lifecycle badges, and the two-arm target / divergence columns — a display-only reordering, detailed in [trade-opportunities.md §Storage and display](trade-opportunities.md#storage-and-display).
...
Because SearXNG is **off the execution gate** (§Connection status), a web-research run *starts* even with no web backend — but a degraded run isn't free (the local suite is SearXNG-only, so it researches blind rather than falling back), and the consequences differ by job, so the app asks for **informed consent before spending the run**.
...
The local suite is **SearXNG-only** ([web-research.md §Tavily fallback](web-research.md#tavily-fallback)), so the wording branches by job alone; there is no fallback, and a degraded run is always flagged *not recommended*:

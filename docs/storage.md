...
- **Run audit record** — each run also stores what it was based on.
  ...
  **Trade Opportunities' contract**, per pick, is **both arms' values side by side**: the model's conviction as authored (no decomposition, no raise field, no app-computed final value — the raise machinery is retired with its two-arm contract) beside the engine arm's **mechanical stand-in**.
  ...
  The research-derived artifacts (retrieval timestamps, distilled findings, source-quality annotations, reuse decisions) and the input delta / per-value attribution land with their producers (the research loop, the input-delta validator) — the conviction-raise machinery is retired on both jobs, so neither stores a raise or an app-derived final conviction — and every Trade Opportunities leg above is designed with that job.

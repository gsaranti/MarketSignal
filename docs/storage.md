...
- **Portfolio Analysis** — per run, the per-holding verdicts: grade and sub-scores, conviction, horizon outlook, price targets — or a fund's typed **`role_risk_only`** assessment where the vehicle class is unpriceable ([portfolio-analysis.md §Asset eligibility](portfolio-analysis.md#asset-eligibility)) — financial-analysis summary, the **portfolio action** with its target-weight range and share/dollar adjustment, and the what-changed audit, split into an intrinsic and an action half, with its per-value cause attribution.
  Each run also stores the per-holding **thesis ledger**: the standing thesis, the bear/base/bull monitor, key falsifiers, and add/trim/sell triggers with a target-weight range — **typed by verdict branch**, since a `role_risk_only` ledger's monitor scenarios are condition-only and its triggers trim / sell only.
  ...
  Each run also stores the portfolio roll-up — carrying, since the construction stage, the **Step-7a aggregates + sizing spine** and the **7b construction view**, so the chosen actions stay auditable against the engine's read they were chosen beside (engine sets + annotation bounds since `portfolio-v7` — [portfolio-analysis.md §Portfolio roll-up and construction](portfolio-analysis.md#portfolio-roll-up-and-construction)).
  Each run row also carries a **`constructed` marker**, authored at the persist seam — true on a normal run, false on a construction-failed degraded run — and mirrored into a store column, so the degraded-run exclusion filters in SQL and no consumer re-derives run health from the roll-up's field shapes ([portfolio-workflow.md §Step 7b](portfolio-workflow.md#step-7b-portfolio-construction)).
  Each holding also carries the **attention-flag state as of this run** (the quick check's non-destructive flag plus which trigger raised it, cleared by the next full pass over the holding, plus any quiet **unexamined-evidence-event** note — overlaid onto the verdict at ledger carry, so the run record holds a *copy*; the live between-run home is the single-row quick-check store below — [portfolio-analysis.md §The quick check](portfolio-analysis.md#the-quick-check-engine-only)).
...
- **Run audit record** — each run also stores what it was based on.
  ...
  It carries the **Portfolio held-name refresh inputs and decisions**: every eligible / selected / skipped holding, its app-owned pre-loop-visible priority fields, exact ledger-node snapshot, typed evidence result, validation result, whether it invalidated reuse / force-included the holding, and any later Step-6b `late-invalidated` slot.
  ...
  **Trade Opportunities' contract**, per pick, is **both arms' values side by side**: the model's conviction as authored (no decomposition, no raise field, no app-computed final value — the raise machinery is retired with its two-arm contract) beside the engine arm's **mechanical stand-in**.
  ...
  It carries **each holding's research-reuse decision** (refreshed vs carried-from-cache, with its vintage) **and each Trade Opportunities pass's reused-vs-freshly-fetched document split** (the document-level cache's audit leg — [trade-opportunities.md §Failure posture](trade-opportunities.md#failure-posture)).
  ...
  The methodology leg records the **run-time FMP `quote` price the targets were computed from** — a transient job-time input logged for traceability, **not** a persisted current-price field; displayed price reads reconstruct from Stooq instead ([trade-opportunities.md §Storage and display](trade-opportunities.md#storage-and-display)).

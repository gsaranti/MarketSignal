...
- **Trade Opportunities** — per run, the 3×3 matrix of opportunities, **plus the persisted opportunity graph — the discovery memory**.
  ...
  It carries the **attention-warning state** (the cheap re-derivation's non-destructive flag plus which trigger raised it — tripwire / upside-exhaustion / re-surfacing — cleared by the next deep pass; [trade-opportunities.md §The opportunity](trade-opportunities.md#the-opportunity)), the **`last_deep_researched_at`** date (driving the *Deep-researched today* affordance and the next deep pass's continuity weight), and its **entry-stamped sector identity** (sector label + resolved SPDR benchmark symbol, refreshed at each deep pass — the since-flagged read's benchmark carrier — [trade-opportunities.md §Outcome learning](trade-opportunities.md#outcome-learning-calibration)).
  ...
  **Every** turn-away class additionally carries the model's **Step-5g record digest** — its conviction, thesis line, and bear case at refusal, so the spread stays sliceable by model conviction.
...
- **Run audit record** — each run also stores what it was based on.
  ...
  **Both jobs' records are two-arm**: each stores the engine arm's values and the model arm's beside them, never merged.
  ...
  Its **picked decision episodes** freeze **both arms'** entry-vintage bands, conviction values, and tier / horizon / runway reads with the admission provenance and both gate vectors into the entry calibration snapshot, so what each arm is graded on is what it actually said at entry — the bands the scored read, the rest recorded unscored for the calibration tier; its **shadow episodes** carry the model's Step-5g record digest (conviction, thesis line, bear case at refusal) beside both arms' gate vectors, so a turn-away preserves what the model wanted rather than only what the gate decided.

...
- **Portfolio Analysis** — per run, the per-holding verdicts: grade and sub-scores, conviction, horizon outlook, price targets — or a fund's typed **`role_risk_only`** assessment where the vehicle class is unpriceable ([portfolio-analysis.md §Asset eligibility](portfolio-analysis.md#asset-eligibility)) — financial-analysis summary, the **rung-only portfolio action** with the action call's rationale (`portfolio-v9`: target-weight ranges, share/dollar adjustments, and the action-half audit are retired; legacy runs' fields stay decodable), and the what-changed audit with its per-value cause attribution.
  ...
  Each run also stores the portfolio roll-up — the deterministic counts and reads plus the data-health aggregate ([portfolio-analysis.md §Portfolio roll-up](portfolio-analysis.md#portfolio-roll-up)); the construction era's aggregates and view blobs survive on legacy rows as decode-only values, and a chosen rung outside the engine's per-holding set is annotated on the holding's audit instead (`action_annotations`).
  Each run row also carries a **`constructed` marker**, authored at the persist seam and mirrored into a store column — every tunnel-vision run persists `true`; the marker survives so the retired construction era's **legacy degraded rows** (`false`) stay excluded from `latest_run` and every baseline read in SQL, with no consumer re-deriving run health from the roll-up's field shapes ([portfolio-analysis.md §Failure posture](portfolio-analysis.md#failure-posture)).
...
- **Trade Opportunities** — per run, the 3×3 matrix of opportunities, **plus the persisted opportunity graph — the discovery memory**.
  ...
  It carries horizon, with its rule-derived `expected_thesis_realization` basis — `dated-catalyst` / `milestone-chain` / `recognition` / `multi-year-compounding` — the engine's `business_runway` durability read, and the validated `runway_evidence` / milestone inputs the derivation read, persisted beside the result for continuity audit.
  It carries the **structured-only and research-informed target sets** plus the validated direct assumptions / `research_target_scenario` expression tree and bridge delta, risk tier, conviction, narrative-vs-reality read, and bear case.
...
- **Run audit record** — each run also stores what it was based on.
  ...
  It carries the **Portfolio held-name refresh inputs and decisions**: every eligible / selected / skipped holding, its app-owned pre-loop-visible priority fields, exact ledger-node snapshot, typed evidence result, validation result, whether it force-included the holding, and any later Step-6b `late-invalidated` slot.
  ...
  **Both jobs' records are two-arm**: each stores the engine arm's values and the model arm's beside them, never merged.
  ...
  It stores the recorded **divergences** too: a rejected archetype overturn with its cited feature and the validator's reason; the advisory tier / horizon / `business_runway` views; and a hard-trigger **status override** — model-proposed status, app-forced status, matched trigger, trigger source lineage — riding the run audit and the pick's episode / archive record, the field set at [trade-opportunities.md §Starting parameters](trade-opportunities.md#starting-parameters-calibratable).
  ...
  Its **picked decision episodes** freeze **both arms'** entry-vintage bands and conviction values with the admission provenance and both gate vectors into the entry calibration snapshot, so each arm is scored on what it actually said at entry; its **shadow episodes** carry the model's Step-5g record digest (conviction, thesis line, bear case at refusal) beside both arms' gate vectors, so a turn-away preserves what the model wanted rather than only what the gate decided.

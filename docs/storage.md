...
- **Portfolio Analysis** — per run, the per-holding verdicts: grade and sub-scores, conviction, horizon outlook, price targets — or a fund's typed **`role_risk_only`** assessment where the vehicle class is unpriceable ([portfolio-analysis.md §Asset eligibility](portfolio-analysis.md#asset-eligibility)) — financial-analysis summary, the **rung-only portfolio action** with the action call's rationale (`portfolio-v9`: target-weight ranges, share/dollar adjustments, and the action-half audit are retired, and the fresh-start ruling (2026-08-17) removed their pre-v9 decode compat — [verification/2026-08-17-fresh-start-legacy-removal.md](verification/2026-08-17-fresh-start-legacy-removal.md)), and the what-changed audit with its per-value cause attribution.
  ...
  Since `portfolio-v45` a quantitative condition's `statement` is app-rendered from its core and the model's short name persists beside it as `label` (`None` on a qualitative condition, whose statement is the model's).
  The render rule is canonical at [portfolio-analysis.md §The position thesis ledger](portfolio-analysis.md#the-position-thesis-ledger).
...
- **Run audit record** — each run also stores what it was based on.
  ...
  **Trade Opportunities' contract**, per pick, is **both arms' values side by side**: the model's conviction as authored (no decomposition, no raise field, no app-computed final value — the raise machinery is retired with its two-arm contract) beside the engine arm's **mechanical stand-in**.
  ...
  The research-derived artifacts are wired with the research loop — source URLs with retrieval timestamps, the distilled findings (the combined object and the reconciled per-topic seed layer, whose claims separately persist retrieval time, search/seed publication metadata, and precision-preserving fact period), source-quality annotations on the ledger claims, the per-topic seeded-vs-cold reuse decisions, the distillation shape, and the forward-assumption resolution — while the conviction-raise machinery stays retired on both jobs (neither stores a raise or an app-derived final conviction), and every Trade Opportunities leg above is designed with that job.

...
4. **Distillation** — the reasoner in non-thinking mode consolidates the loop's findings into candidate summaries. This runs on the same resident 122B by default — one model across the suite avoids per-run model swaps; the fast 35B tier is a benchmark-gated option (see [local-models.md §The model roster and per-task routing](local-models.md#the-model-roster-and-per-task-routing)).

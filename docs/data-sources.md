...
*Result of the #8 endpoint tier audit against the suite's actual paid FMP plan (verified 2026-06-26; `profile` added and live-verified 2026-07-10).
...
Each is an **opt-in addition to the existing scan, not a replacement**: the report's current data-source logic is unchanged, each enrichment soft-degrades to today's behavior on any failure, and all are paid-gated, so they are live-verified together with the suite's paid-key checkpoint — for the calendar enrichment that checkpoint **includes verifying the release→event map's drafted FMP event strings against live calendar responses** (a named blocking prerequisite of wiring it; the row set below is drafted, not verified).
...
The complete **drafted row set** follows — the Stooq-identities convention (drafted in-doc, live-verified before wiring): the **FMP event strings and raw scales are pending paid-key verification**, while polarity follows one stated convention — **activity reads are `higher-favorable`, inflation reads `lower-favorable`, and prints whose market read inverts by regime (wages, job openings — growth-positive but rate-hawkish) are `none`** — and the epsilons are judgment-drafted in-line bands like the corpus's other drafted constants; a wrong event string fails soft to an unmapped event (names + dates), never a wrong classification:

| FRED `release_id` | FRED release (display) | FMP event (string pending verification) | Unit normalization | Polarity | Epsilon |
| --- | --- | --- | --- | --- | --- |
| 50 | Employment Situation | Non Farm Payrolls | thousands of jobs (raw scale verified at checkpoint) | higher-favorable | ±25k |
...
| 46 | Producer Price Index | PPI MoM | % month-over-month | lower-favorable | ±0.2pp |
| 46 | Producer Price Index | Core PPI MoM | % month-over-month | lower-favorable | ±0.2pp |
...
| 192 | Job Openings and Labor Turnover Survey | JOLTS Job Openings | millions of openings | none | ±0.2M |
...
- the performance endpoints return a daily **`averageChange`** (that date's average constituent move) expressed in **percentage points** (`0.64` means 0.64%, the same convention the shipped snapshot adapter's `change_pct` preserves), *not* a price level or a decimal fraction, so the engine normalizes each day to `daily_return = averageChange / 100` and **compounds the normalized daily changes into a trailing cumulative return** (`Π(1 + averageChange / 100) − 1`, serialized as a decimal fraction) — the rising / falling read — rather than percentiling the raw daily moves (a percentile of `averageChange` would say "today's move was unusual," not "the group is up over the window").

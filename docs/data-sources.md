...
The map is the **versioned canonical table** the classification codes against: each row is one **release ↔ event pair** — keyed by the **numeric FRED `release_id`** (the shipped `RELEASES` const's stable key, each display name pinned to its ID) plus the FMP event name, the stable per-event identity (a release's rows form its event set); **code joins on the numeric ID plus the verified event string, never on the release's display name**, which is presentation text only (the shipped `EconomicRelease` carries only the name downstream today, so the join lives inside the FRED adapter's fetch loop — where the ID is in scope — or the struct gains an ID field, settled at wiring) — carrying that event's **unit normalization** between the two feeds, a **date-match tolerance** (same calendar date, ±1 day for cross-midnight publishes — drafted; one shared rule, so it stays out of the per-row columns), a **favorable polarity** (`higher-favorable` / `lower-favorable` / `none` — above-consensus payrolls are favorable; above-consensus CPI is not a "beat"), and an **in-line epsilon** in the event's own units.

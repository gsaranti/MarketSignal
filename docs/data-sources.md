...
| Stooq · daily OHLCV CSV (sector / market benchmarks) | run-level | sector- and market-relative benchmarks (`^spx` + the SPDR sector ETFs — symbols and the FMP-sector mapping single-homed in [§Stooq](#stooq)) → the input delta's technology-event pre-flag and the outcome-learning labels; the per-episode benchmark choice reads each episode's entry-stamped sector identity, never a label-time classification call ([portfolio-analysis.md §Outcome learning](portfolio-analysis.md#outcome-learning-calibration)) |

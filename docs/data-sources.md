...
(1) For the report's tracked market-moving releases, the engine joins FMP's `estimate` / `actual` onto the matching FRED release through a **curated release→event map** — FRED release names ("Employment Situation") and FMP event names ("Non Farm Payrolls") don't string-match, and one release fans out to several FMP events.
The map is the **versioned canonical table** that makes the classification codeable: each row carries the release identity (the FRED release ↔ its FMP event names), the **unit normalization** between the two feeds, a **date-match tolerance** (same calendar date, ±1 day for cross-midnight publishes — drafted), a **favorable polarity** (`higher-favorable` / `lower-favorable` / `none` — above-consensus payrolls are favorable; above-consensus CPI is not a "beat"), and an **in-line epsilon** in the release's own units.
From a matched row the engine computes a deterministic, **neutral relation tag** — `above` / `in-line` / `below` estimate (`in-line` when |actual − estimate| ≤ the row's epsilon) — plus the signed actual-vs-estimate **% gap** (`(actual − estimate) / |estimate|`, **`None` when the estimate is zero** — the absolute gap is always carried), and a **beat / miss** read **only where the row maps a polarity** (beat = the favorable side): the engine derives, the model interprets.
...
**SEC EDGAR is retained as the authoritative primary-source cross-check** for the numbers that drive grades / targets, which makes **ticker→CIK resolution a non-blocking enhancement** rather than a prerequisite (the FMP feeds are symbol-keyed).

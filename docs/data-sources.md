...
The symbol-keyed **insider and congressional** positioning signals come from FMP's structured endpoints rather than EDGAR's CIK-keyed filings, so every EDGAR leg rides on **ticker→CIK resolution** (SEC's `company_tickers.json`; today only a hardcoded handful resolve).
The full cached-`company_tickers.json` resolver is a **named prerequisite of the full Portfolio slice** — the per-holding filing checks, the quick check's EDGAR sweep, and the material-filing evidence event all depend on it ([portfolio-analysis.md §The quick check](portfolio-analysis.md#the-quick-check-engine-only)) — while for Trade Opportunities' cross-check it stays a non-blocking enhancement.
...
**FMP** — the per-holding / per-fund endpoints Portfolio Analysis calls on the current plan.
...
| `analyst-estimates` | per-holding | forward revenue / EPS consensus → engine **revision-velocity** read, the quick check's **revision preflight**, and the **v2 target ladder's driver source** (the NTM read — the two nearest forward annual rows time-weighted by twelve-month overlap, mid / low / high per leg, EPS else forward revenue per share — [portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable)) |
...
| Schwab · `/marketdata/v1/chains` | per-holding (optionable equity) | option chains → options-activity signal (put/call, IV/skew); also the greek source for a same-underlier option overlay — a **standalone** option (no held underlier) gets no chain fetch, so its delta is a typed gap |
| SEC EDGAR · submissions (10-K / 10-Q / 8-K) | per-holding (CIK-gated) | filings — authoritative cross-check + 8-K events, item-classified reads feeding the hard-forensic filing kinds ([trade-opportunities-workflow.md §Step 5c](trade-opportunities-workflow.md#step-5c-deterministic-analysis-archetype-weighted-engine)); rides the Portfolio-slice `company_tickers.json` resolver, an unresolved CIK degrading to a typed unknown ([§SEC EDGAR](#sec-edgar)) |

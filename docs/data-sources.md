...
The map is the **versioned canonical table** the classification codes against: each row is one **release ↔ event pair** — keyed by the FRED release identity plus the FMP event name, the stable per-event identity (a release's rows form its event set) — carrying that event's **unit normalization** between the two feeds, a **date-match tolerance** (same calendar date, ±1 day for cross-midnight publishes — drafted; one shared rule, so it stays out of the per-row columns), a **favorable polarity** (`higher-favorable` / `lower-favorable` / `none` — above-consensus payrolls are favorable; above-consensus CPI is not a "beat"), and an **in-line epsilon** in the event's own units.
The tracked releases are the calendar's seven curated market-moving releases (the shipped `RELEASES` table: Employment Situation, Consumer Price Index, Producer Price Index, Personal Income and Outlays, Advance Retail Sales, Job Openings and Labor Turnover Survey, Gross Domestic Product — FOMC arrives separately via the FMP `impact` filter below and needs no map row).
...
| FRED release | FMP event (string pending verification) | Unit normalization | Polarity | Epsilon |
| --- | --- | --- | --- | --- |
| Employment Situation | Non Farm Payrolls | thousands of jobs (raw scale verified at checkpoint) | higher-favorable | ±25k |
| Employment Situation | Unemployment Rate | percent | lower-favorable | ±0.1pp |
| Employment Situation | Average Hourly Earnings MoM | % month-over-month | none | ±0.1pp |
| Consumer Price Index | CPI MoM | % month-over-month | lower-favorable | ±0.1pp |
| Consumer Price Index | Core CPI MoM | % month-over-month | lower-favorable | ±0.1pp |
| Consumer Price Index | CPI YoY | % year-over-year | lower-favorable | ±0.1pp |
| Producer Price Index | PPI MoM | % month-over-month | lower-favorable | ±0.2pp |
| Producer Price Index | Core PPI MoM | % month-over-month | lower-favorable | ±0.2pp |
| Personal Income and Outlays | PCE Price Index MoM | % month-over-month | lower-favorable | ±0.1pp |
| Personal Income and Outlays | Core PCE Price Index MoM | % month-over-month | lower-favorable | ±0.1pp |
| Personal Income and Outlays | Personal Income MoM | % month-over-month | higher-favorable | ±0.2pp |
| Personal Income and Outlays | Personal Spending MoM | % month-over-month | higher-favorable | ±0.2pp |
| Advance Retail Sales | Retail Sales MoM | % month-over-month | higher-favorable | ±0.3pp |
| Advance Retail Sales | Retail Sales Ex Autos MoM | % month-over-month | higher-favorable | ±0.3pp |
| Job Openings and Labor Turnover Survey | JOLTS Job Openings | millions of openings | none | ±0.2M |
| Gross Domestic Product | GDP Growth Rate QoQ | % annualized quarter-over-quarter | higher-favorable | ±0.3pp |
| Gross Domestic Product | GDP Price Index QoQ | % annualized quarter-over-quarter | lower-favorable | ±0.2pp |
...
Two **maintenance populations** additionally ride a subset of the same rows outside the discovery funnel, scaling with the live matrix and episode population rather than the discovery budget (tagged inline below): the **carried-matrix / watchlist sweep** — Step 3c's watchlist re-check and Step 7's cheap re-derivation, spending `quote` / `analyst-estimates` / `earnings` plus Stooq daily bars per live opportunity through the shared price-bar cache — and the **label-time outcome refresh** — Stooq bars through the window end per maturing episode, **picked and shadow alike** (all five turn-away classes — the shadow ledger is plausibly the larger label-time population), counted **per distinct symbol after dedup** through the shared price-bar cache; the per-run population is only the episodes with a window maturing since the last run, never the whole bounded ledger.
The sweep additionally carries a **conditional filing-cadence rider**: the swept `earnings` row doubles as the fresh-filing trigger, and a name whose latest reported period postdates the last sweep refreshes its statement-derived rows — the core statements, `key-metrics` / `ratios`, `financial-scores`, and `financial-growth` (tagged inline below) — for that sweep only, per-name and ~quarterly in practice, so the rider stays small against the per-candidate budget.
...
| `analyst-estimates` | per-candidate + per live opportunity (Step-7 cheap sweep) | forward consensus, snapshotted run-to-run → engine **revision-velocity** read |
...
| `earnings` | per-candidate + per live opportunity (Step-7 cheap sweep) | next earnings date (catalyst) + actual-vs-estimate surprise / SUE history |
...
| `quote` | per-candidate + per live opportunity (Step-7 cheap sweep) + per selected name (ATO Quick Audit) | live quote (current price) |
...
| Stooq · daily OHLCV CSV (equities) | per-candidate + per live opportunity (Step-7 cheap sweep, via the shared price-bar cache) + per maturing picked **or shadow** episode (label time, symbol-deduped) | deep price history → price-action confirmer (relative strength / base breakout), momentum / volatility, scenario targets |
| Stooq · daily OHLCV CSV (futures, incl. copper `hg.f` — symbols single-homed in [§Stooq](#stooq)) | run-level | daily commodity-price turn for the cyclical sleeve (complements the monthly IMF series) |

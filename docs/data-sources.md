...
Base `https://api.stlouisfed.org/fred`; two endpoints — `/series/observations` (the latest level of each series — the `series_id` doubles as the quote symbol) and `/release/dates` (the economic-release calendar schedule).
...
| SEC EDGAR · submissions (10-K / 10-Q / 8-K) | per-holding (CIK-gated) | filings — authoritative cross-check + 8-K events; rides the Portfolio-slice `company_tickers.json` resolver, an unresolved CIK degrading to a typed unknown ([§SEC EDGAR](#sec-edgar)) |

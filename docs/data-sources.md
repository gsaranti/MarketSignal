...
| FINRA · consolidated short-interest file | per-holding lookup (file fetched once / run) + **conditional quick-check lookup** (the same once-per-run file, fetched only when a holding carries a short-interest-fed validated quantitative condition — [portfolio-analysis.md §The quick check](portfolio-analysis.md#the-quick-check-engine-only)) | short-interest level / trend / days-to-cover → risk / squeeze context; the conditional leg keeps a short-interest ledger condition machine-evaluable between runs |

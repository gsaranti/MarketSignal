...
Each is an **opt-in addition to the existing scan, not a replacement**: the report's current data-source logic is unchanged, each enrichment soft-degrades to today's behavior on any failure, and all are paid-gated, so they are live-verified together with the suite's paid-key checkpoint.
...
The map is the **versioned canonical table** that makes the classification codeable: each row is one **release ↔ event pair** — keyed by the FRED release identity plus the FMP event name, the stable per-event identity (a release's rows form its event set) — carrying that event's **unit normalization** between the two feeds, a **date-match tolerance** (same calendar date, ±1 day for cross-midnight publishes — drafted), a **favorable polarity** (`higher-favorable` / `lower-favorable` / `none` — above-consensus payrolls are favorable; above-consensus CPI is not a "beat"), and an **in-line epsilon** in the event's own units.
...
| Stooq · daily OHLCV CSV (equities) | per-candidate + per live opportunity (Step-7 cheap sweep, via the shared price-bar cache) + per maturing picked episode (label time) | deep price history → price-action confirmer (relative strength / base breakout), momentum / volatility, scenario targets |

...
This file lists the external data and model providers the application depends on.
Credential configuration for these providers is covered in [configuration.md](configuration.md).
...
- **Trade Opportunities** — discovers new ideas across a 3×3 risk×horizon matrix; local models reason over a discovery → per-candidate funnel ([trade-opportunities.md](trade-opportunities.md)).
  Its second trigger, **Audit**, re-evaluates user-selected matrix names under the same one `trade_opportunities` job identity ([trade-opportunities.md §The two jobs](trade-opportunities.md#the-two-jobs)).
...
| --- | --- | --- | --- |
| [FMP](#financial-modeling-prep) *(paid key)* | baseline equity-market scan | per-holding fundamentals / revision / positioning / segments / quote + deep price history & benchmark series | discovery (screener-stratify + event feeds) **+** per-candidate validation + deep price / benchmark / commodity series |
...
| [SEC EDGAR](#sec-edgar) | — | authoritative cross-check (10-K/Q/8-K + XBRL) | authoritative cross-check + 8-K; S-1 / Form 10 history for app-eligible limited-history candidates |
...
This part catalogs every provider — what it is and why the app uses it.
**Which** endpoints each job calls is in [Endpoints by job](#endpoints-by-job); plan-tier status lives here, under FMP, since it is a property of the source.
FMP, FRED, and CFTC feed **all three jobs**; BLS, GDELT, and the cloud LLMs are report-only; Schwab, SEC EDGAR, FINRA, CBOE, and SearXNG are local-suite-only.

The application accesses market and financial data by calling provider REST APIs directly from the Rust backend (`reqwest`/`serde`).
**Financial Modeling Prep** is the primary financial-data source, supplying equity-market data — indices, volatility, gold and silver, sector performance, multi-horizon index returns, market movers, the earnings calendar, sector and industry valuation (P/E) plus finer-grained industry rotation, and the equity-risk-premium.
**FRED** and **BLS** supply macroeconomic and labor data through their public APIs; FRED additionally provides the US dollar index, commodity prices (oil, natural gas), Treasury yields, credit and yield-curve spreads, and financial-conditions indices.

**Every date that keys a session-scoped read dates on the ET session, not the UTC calendar day** — the cross-cutting rule, applied across both chains (`market_clock::et_session_date` / `et_date_of`; swept whole 2026-08-07, [verification/2026-08-07-scoped-conformance-check.md](verification/2026-08-07-scoped-conformance-check.md) §Rulings).
That covers a date-keyed snapshot's candidate date (`sector-pe-snapshot` and its sector / industry siblings), any staleness bound compared against a provider's own observation date (the FRED rate-anchor freshness floor and the report scan's per-series guards, the quick check's rate-cache age), a report's age at the house-view freshness gate, the reference date the forward-estimates blend selects rows from, the trailing-twelve-month dividend window, and every evidence boundary or evaluation stamp ([portfolio-analysis.md §Evidence floor](portfolio-analysis.md#evidence-floor), [§Outcome learning](portfolio-analysis.md#outcome-learning-calibration)).
Because provider observation dates are market days, a UTC read on an evening-ET run — after ~8 PM EDT, when the UTC date has rolled but the session has not — asks for a session that never traded or ages a print by one day, and a date-keyed snapshot answers **200 with an empty array** rather than an error, so the miss is silent.
The deliberate exception is a **fetch range's own bounds** — either end: those stay on the UTC date, because a range selects a *window of data* rather than keying a session, so a range ending one day late simply serves no row for the untraded day and one starting a day early widens a multi-day lookback by one. Each such site is annotated as left alone on purpose (the EOD, company-history, deep-history, anchor-window and dated-EOD ranges, the price-bar fetch bound and its lookback count, and Tavily's cadence-sized news recency `start_date`).
One range is deliberately **not** left there, for a reason outside this rule: the Schwab option-chain window is ET-dated because it previously read the *machine's* local date, so the same code selected a different sixty-day expiration window on a Pacific-time machine than on an Eastern one.
That is a per-machine read rather than a per-session one — the fix is determinism, not session dating, and the range-bound exception still holds for every UTC-dated range.

The gated REST adapters (FMP, FRED, BLS, Tavily) share a bounded retry-with-backoff for transient HTTP-status/transport failures (HTTP 429, 5xx, dropped connections; `Retry-After`-aware), parameterized by a per-provider policy (`http_retry::RetryPolicy`) whose default preserves the short shared schedule.
**FMP carries its own policy** (`fmp::FMP_RETRY`, probe-verified 2026-08-05: the paid plan's per-minute limit arrives as an HTTP 429 with the `"Limit Reach"` body and no `Retry-After`): its 429 ladder doubles 1 s → 32 s — 63 s cumulative over seven attempts, guaranteed to cross any minute window — while 5xx keeps the short schedule so a down provider fails fast, a server `Retry-After` is honored up to 90 s, and backoff sleeps poll the run's cancel flag in ~250 ms slices (both FMP clients, the baseline adapter and the news leg, ride it; the Settings connection test deliberately stays single-shot).
GDELT is deliberately excluded — its escalating per-IP lockout makes a retry harmful, so it keeps its single-shot fail-soft.
...
**Used by:** Report · Portfolio Analysis · Trade Opportunities — the data spine of all three.
The endpoints each job calls are listed in [Endpoints by job](#endpoints-by-job); plan-tier status for every FMP path any job calls is in the [audit below](#fmp--current-paid-plan-tier-audit).
...
(They lived with the removed keyless price source until 2026-08-12 — [verification/2026-08-12-stooq-removal-decision.md](verification/2026-08-12-stooq-removal-decision.md).)
The **broad-market benchmark is `^GSPC`** (the S&P 500 price index).
The **sector benchmarks are the SPDR Select Sector ETFs** — ETF proxies rather than S&P sector indices — mapped from FMP's sector labels as Technology → `XLK`, Financial Services → `XLF`, Energy → `XLE`, Healthcare → `XLV`, Industrials → `XLI`, Consumer Defensive → `XLP`, Consumer Cyclical → `XLY`, Basic Materials → `XLB`, Utilities → `XLU`, Real Estate → `XLRE`, Communication Services → `XLC`.
The **commodity identities are FMP's commodity symbols** — copper `HGUSD`, gold `GCUSD`, silver `SIUSD` — Trade Opportunities' canonical daily gold / silver price context ([§Trade Opportunities — endpoint surface](#trade-opportunities--endpoint-surface)); gold is live-verified, and the silver / copper endpoint shapes are verified at Trade Opportunities build time.
`historical-price-eod/light` — the suite's only deep price source — serves **split-adjusted, dividend-unadjusted** daily bars, exactly the price-only basis the suite's labels and relative reads require, desk-verified 2026-08-02 against FMP's own `non-split-adjusted` and `dividend-adjusted` variants over the NVDA 2024 split and Altria's cumulative payout ([verification/2026-08-02-fmp-light-eod-adjustment-basis.md](verification/2026-08-02-fmp-light-eod-adjustment-basis.md)).
The label-time semantics (price-only common basis on both sides, dividends added separately from a label-time `dividends` re-pull) are single-homed in [portfolio-analysis.md §Outcome learning](portfolio-analysis.md#outcome-learning-calibration).
Consumers reference this identity table rather than restating it.

#### FMP — current paid-plan tier audit

*Result of the #8 endpoint tier audit against the suite's actual paid FMP plan (verified 2026-06-26; `profile` added and live-verified 2026-07-10; the key upgraded onto the plan 2026-07-16, with live spot-checks confirming the buckets — allowed → 200, blocked → 402, and the `analyst-estimates` annual-only constraint verbatim).
It covers **every FMP endpoint the report and both local jobs call** — the per-job endpoint surfaces in [Endpoints by job](#endpoints-by-job) below (Market Signal Report, Portfolio Analysis, Trade Opportunities) — sorted into three buckets: **allowed (with constraint)**, **blocked → fallback**, **blocked → no fallback**.
Net: the **report is fully covered**; **Portfolio equity grading is clean**; **Portfolio funds degrade** (ETF constituent look-through lost; mutual-fund **constituent holdings** have no on-plan FMP path, while fund information and available allocation data remain usable); **Trade Opportunities needed a discovery-funnel redesign** — every `*-bulk` endpoint is blocked (since done: the discovery layer below is redesigned around `company-screener`-stratified discovery + per-symbol scoring on the budget-narrowed candidate slate).*

**Allowed (with constraint).**
Every US-exchange limit is moot — the suite analyzes US listings only, **enforced rather than assumed**: a Schwab account can hold foreign ordinaries and other non-US listings, and the Portfolio eligibility contract's listing-resolution guard routes those to not-rated before they reach these endpoints ([portfolio-analysis.md §Asset eligibility](portfolio-analysis.md#asset-eligibility)); the report side never leaves US aggregates.
...
| --- | --- |
| **All bulk** — `scores-bulk`, `earnings-surprises-bulk`, `ratios-ttm-bulk`, `key-metrics-ttm-bulk`, `rating-bulk`, `upgrades-downgrades-consensus-bulk`, `price-target-summary-bulk`, `income-statement-growth-bulk`, `cash-flow-statement-growth-bulk`, `dcf-bulk` | `company-screener` does the universe-wide first cut; the multi-factor composite (forensic / surprise / rating-flow / growth / DCF-gap) is computed **per-symbol on the budget-narrowed candidate slate** — the Step-4 selection, never the screener longlist (see the Trade Opportunities discovery funnel in [trade-opportunities.md](trade-opportunities.md)). The discovery-breadth budget governs how many names reach per-symbol scoring. |
...
**Used by:** Report (macro / rates / credit backbone) · Portfolio Analysis (run-level risk-free + commodity context) · Trade Opportunities (run-level risk-free + commodity-cyclical sleeve).
Each job's series subset is listed in [Endpoints by job](#endpoints-by-job).

FRED provides official macroeconomic and financial data maintained by the Federal Reserve Bank of St. Louis.
Base `https://api.stlouisfed.org/fred`; two endpoints — `/series/observations` (the latest level of each series — the `series_id` doubles as the quote symbol; the local suite's one **dated-history** request, the v2 anchor-window `DGS10` series, rides the same endpoint's date-ranged form — the Portfolio DGS10 row in [Endpoints by job](#endpoints-by-job)) and `/release/dates` (the economic-release calendar schedule).
FRED's documented limit is 120 requests/minute with no daily cap; each report run's ~40-request scan sits far under it.
...
**Used by:** Market Signal Report only (CPI / payrolls / wages).
Neither local job calls BLS.
...
The CFTC's weekly Commitments of Traders report supplies the one signal the price, valuation, macro, and credit groups can't: how crowded or extended the *speculative* cohort is in the market's bellwether futures.
It is the application's positioning input, accessed through the CFTC public-reporting Socrata API, which is **keyless** — like BLS, it needs no credential and sits outside the execution gate.
...
- **Traders in Financial Futures** (dataset `gpe5-46if`) — for the equity-index, rates, and FX contracts.
  Its leveraged-money ("fast money") and asset-manager ("real money") split is the signal: the two cohorts often diverge (real money net long while leveraged money presses shorts).
- **Disaggregated** (dataset `72hh-3qpy`) — for the commodity contracts.
  Managed money is the speculator proxy; there is no asset-manager cohort, so the real-money line is omitted.
...
Each row carries the speculator net (long − short), its week-over-week change, the speculator long as a percent of open interest, and — for financial futures — the asset-manager net and its change.
The data is weekly: a Tuesday snapshot released the following Friday, so a report always reads the prior week's positioning, and each row carries its snapshot date so the model reads it as-of.
A bounded freshness guard drops a row older than three weeks (a stalled feed) to a gap rather than presenting it as current.
The group is fail-soft and additive — a flaky contract or a whole-API outage degrades to a recorded gap rather than failing the run, and it carries no coverage floor.
Because COT already carries its own native week-over-week change and follows a fixed weekly cadence, the positioning group is exempt from the report-over-report baseline change view.
...
The first report (no prior interval) omits the bound and takes Tavily's own default.
The Step-9 research executor's plan queries carry no recency bound — they target a topic, not a time slice.

### FMP Articles
Docs - https://site.financialmodelingprep.com/developer/docs (Articles)

**Used by:** Report (Step-7 news funnel) · Trade Opportunities (discovery theme / event scan).
Not Portfolio Analysis.
Reuses the shared FMP key — free-tier but authenticated, never keyless (the discovery scan's keyless leg is the SearXNG web tool, not this feed).

FMP Articles is Financial Modeling Prep's in-house editorial feed: short, ticker-tagged market commentary written against FMP's own data (analyst-coverage moves, earnings reactions, notable price action).
It is the one FMP news surface on the free tier (verified live; the third-party `news/*` feeds are premium — see [Financial Modeling Prep](#financial-modeling-prep) above), and reuses the FMP API key already required for market data.
...
The application gathers a single bounded page of recent articles per run, alongside Tavily's topic sweep and GDELT's geopolitical query.
The feed is best-effort: it is house-written and overlaps what Tavily already indexes, so the deterministic dedup pre-pass and the headline-filter model absorb redundancy, and a failing gather degrades to no headlines rather than failing the job — the same fail-soft posture as GDELT.

### GDELT
Docs - https://www.gdeltproject.org/data.html

**Used by:** Market Signal Report only (geopolitical sweep).
**Dropped from the local suite** — its escalating IP-lockout makes it unreliable (the same reason it stays single-shot fail-soft here; see the [Trade Opportunities endpoint surface](#trade-opportunities--endpoint-surface)).
...
GDELT's single combined query sizes its `timespan` lookback to the **elapsed interval since the previous report** (rounded up to whole days, clamped to a floor and a one-month cap), rather than a fixed week — keeping the geopolitical feed matched to the on-demand cadence.
The first report (no prior interval) uses a one-week default.
This changes only the window width, not the request count: it stays a single bounded query, so GDELT's burst rate limit is unaffected.
...
**Used by:** Portfolio Analysis · Trade Opportunities (local suite only) — **a connected Schwab account is required to run either local job**.
Not the report.

Charles Schwab is the source of the user's **portfolio holdings** *and* **equity option chains** for the local suite — positions (quantity, cost basis, market value, instrument identity) plus `cashBalance` and `liquidationValue` via the accounts endpoint, and live option chains via the same OAuth's market-data endpoint (`/marketdata/v1/chains`, returning per-contract volume, open interest, implied volatility, and greeks).
The holdings adapter reconciles any explicit cash-equivalent / currency position against the two balance fields and folds it into cash exactly once; an ambiguous overlap fails the pull rather than inflating the account-total denominator ([schwab-integration.md §What is pulled](schwab-integration.md#what-is-pulled)).
It is not a fundamentals source — Schwab's fundamentals are thin, so company financials come from FMP and SEC EDGAR (below).
From the chains the suite computes a deterministic **options-activity signal** — put/call ratio (by volume and open interest) plus an IV/skew read (a rough activity proxy, not positioning truth — see [schwab-integration.md](schwab-integration.md)).
A connected Schwab account is **required to run either local job**; authentication, token lifecycle, account hashing, and the manual-import supplement are described in [schwab-integration.md](schwab-integration.md).

### SEC EDGAR
Docs - https://www.sec.gov/edgar/sec-api-documentation

**Used by:** Portfolio Analysis · Trade Opportunities (local suite only) — the authoritative cross-check behind the FMP working feed.
Not the report.

SEC EDGAR is the **authoritative source for company filings and fundamentals** behind Portfolio Analysis (and candidate validation in Trade Opportunities): 10-K / 10-Q / 8-K submissions, the XBRL **company-facts** API for normalized financial-statement data, and — for an app-eligible Trade Opportunities new listing / separation — the same submissions surface's S-1 / Form 10 filing history.
It is **keyless** (like BLS and CFTC), requiring only a declared User-Agent with contact info per the SEC's fair-access policy, and is rate-limited to ~10 requests/second.
SEC supplies the raw statement data the deterministic financial-analysis engine ([portfolio-analysis.md](portfolio-analysis.md)) computes over; FMP remains for convenient normalized metrics and market-data signals.
The company-facts read is deliberately the **annual fallback basis** behind FMP's TTM statement basis ([portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable)): the latest 10-K full-year row per GAAP concept, the prior-year revenue taken from the **same concept's** second-latest distinct period end (never a different revenue tag), duplicate comparative rows collapsed.
**Fund holdings skip the facts call entirely** — a wrapper's statement lines feed nothing on the reduced path, and the trust entity behind an ETF routinely 404s the facts API into pure audit gap noise (the 2026-07-31 run's QQQ finding).
Authoritative filings reduce the suite's dependence on web-search summaries for the numbers that drive grades and targets.

For Trade Opportunities, EDGAR is the **authoritative cross-check** behind the FMP working feed — the final grade / target numbers are reconciled against EDGAR filings and XBRL (compute-don't-guess), 8-K filings remain a primary source, and S-1 / Form 10 history may enter only through the limited-history identity / perimeter / comparability validation at [trade-opportunities.md §Evidence floor](trade-opportunities.md#evidence-floor).
The submissions feed's 8-K reads are **item-classified**: an Item 4.01 (auditor change) or Item 4.02 (non-reliance restatement) filing is what feeds the hard-forensic producer's filing kinds, the contract single-homed at [trade-opportunities-workflow.md §Step 5c](trade-opportunities-workflow.md#step-5c-deterministic-analysis-archetype-weighted-engine).
The symbol-keyed **insider and congressional** positioning signals come from FMP's structured endpoints rather than EDGAR's CIK-keyed filings, so every EDGAR leg rides on **ticker→CIK resolution** (SEC's `company_tickers.json`).
The full cached-`company_tickers.json` resolver is **built and live** — the whole map parsed and disk-cached with a staleness bound, fail-soft to a stale cache — serving the per-holding filing checks, the quick check's EDGAR sweep, and the material-filing evidence event ([portfolio-analysis.md §The quick check](portfolio-analysis.md#the-quick-check-engine-only)); for Trade Opportunities' cross-check it stays a non-blocking enhancement.
A symbol the resolver cannot match, or matches ambiguously (ADRs, class shares, recent renames), yields a **typed unresolved-CIK unknown**: that holding's EDGAR legs degrade to the FMP working feed and the unknown feeds the degraded-sweep state ([portfolio-analysis.md §Triggering](portfolio-analysis.md#triggering)) rather than silently reading as a clean check.
Portfolio's bounded issuer-IR recovery also reads current submissions metadata, the filing document table, the primary 8-K and a matched earnings Exhibit 99.1 under the research runner's existing budget.
It supplies cited research evidence only; it does not change company-facts, the forensic classifier or deterministic financial inputs.
Its eligibility, matching, dates, request bounds and partial-coverage contract are canonical at [web-research.md §Portfolio earnings-release recovery](web-research.md#portfolio-earnings-release-recovery).
**Institutional 13F** is the exception: it is off-plan on the current FMP plan ([§FMP — current paid-plan tier audit](#fmp--current-paid-plan-tier-audit)), and because EDGAR 13F is filer-keyed, a per-name read is only coarse and optional (often omitted) rather than a clean symbol-keyed call.

### FINRA (short interest)
Docs - https://www.finra.org/finra-data/browse-catalog/equity-short-interest

**Used by:** Portfolio Analysis (held-equity risk / squeeze context) · Trade Opportunities (short-interest extremes screen + per-candidate).
Local suite only — not the report.

FINRA publishes the **consolidated Equity Short Interest** file — keyless, biweekly (mid- and end-of-month settlement, disseminated ~7–8 business days later), covering exchange-listed and OTC equities since 2021.
Each record carries current and prior short interest plus average daily volume, giving short-interest level, trend, and days-to-cover directly — a **bearish-by-default** factor for Trade Opportunities (heavily-shorted names underperform on average), flipping to a *conditional squeeze* read only when paired with an inflecting leading metric + near-term catalyst + a breaking bear thesis ([trade-opportunities.md §The research method](trade-opportunities.md#the-research-method)), **and a risk / squeeze-context input for Portfolio Analysis's held equities** (informing trims and adds; the biweekly file is fetched once per run and looked up per holding).
It is best-effort and fail-soft like the suite's other additive feeds.
**Retrieval is a two-step keyless fetch** (built, live-verified 2026-08-21): the latest settlement date is **discovered, never computed locally** — FINRA designates it (mid-month is the 15th *or the preceding business day*, end-of-month the *last business day*) — then the static CDN file `cdn.finra.org/equity/otcmarket/biweekly/shrt{YYYYMMDD}.csv` is fetched whole (~22k rows, ~2 MB).
Discovery prefers the keyless partitions endpoint (`api.finra.org/partitions/group/otcMarket/name/consolidatedShortInterest` — it answered without credentials when live-verified, though FINRA's docs describe an OAuth model), falling back to scanning the public files page for `shrt########.csv` links, so an enforcement change on the API shifts the discovery route without touching the file fetch.
The file is **pipe-delimited despite the `.csv` extension**, columns resolved from the header row by name; a missing required column is structure drift, a typed gap, never an empty success.
Per-holding lookup follows FINRA's reporting convention by uppercasing the account symbol and removing spaces and special characters at the adapter boundary (`BRK/B` → `BRKB`); the original Schwab symbol remains the holding's identity everywhere else.
A not-yet-published date answers HTTP **403**, not 404 — any non-2xx reads as not-published.
Watch: SR-FINRA-2026-012 (filed May 2026) proposes weekly reporting with faster publication; approval may change the cadence and possibly the file naming.

### CBOE
Docs - https://www.cboe.com/us/options/market_statistics/daily/

**Used by:** Portfolio Analysis · Trade Opportunities (local suite only) — a venue-level options-sentiment backdrop.
Not the report.

CBOE provides a free, keyless daily **put/call ratio for its own exchange's flow** (total, equity, index) — a Cboe **venue-level** options-sentiment backdrop (not consolidated all-market data, which is a separate paid dataset), distinct from the per-stock put/call the suite computes from Schwab chains.
It is an optional broad-market context signal read from Cboe's daily statistics, fail-soft and venue-level only (no per-symbol breakdown), so it informs the macro sentiment read rather than any single verdict.
**Retrieval is a bounded page extraction, not an API call**: no machine-readable current-day endpoint exists (verified 2026-08-20 — the cdn.cboe.com ratio CSVs froze at 2019-10-04), and the daily page is a client-rendered app whose raw response embeds the day's statistics as a JSON payload, so the adapter scans that payload for the ratio labels and the `selectedDate` key, and locally detectable structure drift — wrong key or label shape, wrong quote representation, disagreeing same-form candidates — degrades to a typed gap rather than a fabricated ratio.
The gap-over-fabrication guarantee is deliberately scoped to locally detectable drift.
Parsing the payload whole would mean first reassembling the chunk-split script stream — a heavier retrieval strategy declined by ruling 2026-08-21 in favor of the bounded scan.
A well-formed sole impostor in the payload's own form is accepted residual risk, sized to the backdrop's optional venue-context role.
The `selectedDate` the extraction reads rides as the backdrop's as-of, under the same guarantee scope.

### LLM Providers
- [OpenAI](https://platform.openai.com/docs)
- [Anthropic](https://platform.claude.com/docs)

**Used by:** Market Signal Report only (the four user-configurable agent models + the fixed internal stages).
The **local analysis suite runs entirely on local open-weight models** served on-device ([local-models.md](local-models.md)), so neither local job calls these providers.

The specific models exposed by these providers, the user-configurable model selections for each agent, and the API-token requirements are covered in [configuration.md](configuration.md).
The non-configurable models used by fixed internal pipeline stages are covered in [agents.md](agents.md).
The local analysis suite uses local open-weight models served on-device instead of these providers — see [local-models.md](local-models.md).

## Source registry and evidence tiers

The web-research loop ([web-research.md](web-research.md)) reaches an open, unbounded set of domains, so the suite governs *which* sources it trusts for *what* through a **source registry** — structured per-domain metadata the orchestrator consults at search-rank and fetch time.
The registry is **not a whitelist and not exhaustive**: it is a **thin override over heuristic defaults** — the long tail of the web is never registered and is scored by default rules, while the registry pins the handful of domains whose treatment must be deliberate (primary sources, known specialists, denied junk, connected subscriptions).

**Per-domain metadata.**
Each registered domain carries **`tier`** (its evidence tier, below).
It carries **`evidenceKinds`** — *what the source is good for*, not one global score (`["filings","financials","legal-disclosure"]` for SEC, `["event-verification"]` for a wire, `["semis-supply-chain"]` for SemiAnalysis) — so a source can be Tier-0 for one kind and weak for another (SEC is authoritative for reported financials, useless for market narrative), and weighting is **per evidence-kind**, never one reputation number.
It carries **`lanePolicy`** — how the source is treated per research lane ([web-research.md §Source quality and evidence weighting](web-research.md#source-quality-and-evidence-weighting)) — and **`credentialMode`**, either `none` (public) or `connected` (an authenticated subscription — [web-research.md §Connected sources](web-research.md#connected-sources-authenticated-fetch)).
It carries **`freshnessSlaDays`** — how stale a hit may be before it is down-weighted for this kind (long for a filing, short for an ASP read); **`extractionProfile`**, how the fetch layer should treat the domain (`api_or_html` / `html` / `js_required`), seeded by extraction telemetry; and **`paywall`**, whether full text depends on a connected subscription.

**Evidence tiers.**
Every source resolves to a tier — registered explicitly, or by default heuristic:
- **Tier 0 — primary / deterministic:** SEC EDGAR, company IR + earnings releases, 10-K/Q/8-K, the Fed / FRED / BLS / BEA / EIA / FDA / CFTC / FINRA, court & regulator documents.
  The compute-don't-guess anchor.
...
- **`deny`:** SEO stock mills, AI-generated quote pages, thin affiliate sites, press-release-repost spam — dropped at search-filter and fetch-gate.
  This is the **one categorical exclusion**, distinct from the tier gradient (0–5), which only *weights* evidence and never drops it — denying junk that isn't evidence at all is not gating on quality ([web-research.md §Source quality and evidence weighting](web-research.md#source-quality-and-evidence-weighting)).

The tiers are seeded with a **representative starter set, not a maintained catalog of the whole web**: an unregistered domain is scored by default heuristic (a recognized `.gov` / primary-filing host defaults high, an unknown blog low), so the system degrades gracefully rather than depending on registry completeness.
How tiers and evidence-kinds translate into ranking and conviction — and the load-bearing rule that they **inform, never gate** — is in [web-research.md §Source quality and evidence weighting](web-research.md#source-quality-and-evidence-weighting).
...
The application calls Financial Modeling Prep directly for the equity-market portion of the baseline market-data scan ([report-workflow.md §Step 3](report-workflow.md#step-3-gather-baseline-market-data)) — indices, volatility, gold and silver, and sector performance.
Each index's multi-horizon performance (weekly, month-to-date, year-to-date, and 52-week-range position) is derived from FMP's free historical end-of-day prices (verified live: the indices and the VIX return on the free tier).
FMP also supplies the **market movers** (biggest gainers / losers / most-active names, filtered to major-exchange names above a price floor, with leveraged / inverse ETFs excluded, capped per list).
It supplies the free **earnings calendar** — the recently reported and upcoming large-cap reporters, on FMP's ~1-month free window, filtered by revenue estimate; the recently-reported lookback is sized to the report cadence, so a monthly run sees the whole interval's reporters rather than just the last week, while the upcoming window stays fixed.
It supplies the free **valuation + finer-rotation** snapshots: per-**sector P/E** (a valuation read alongside sector performance), the strongest and weakest **industries** (FMP reports ~130 per exchange, capped to the extremes; each industry's average move joined with its aggregate P/E where available), and the US **equity-risk-premium** (from FMP's per-country market-risk-premium dataset, a near-static annual constant).
...
Two endpoints: `/series/observations` (latest level of each series below — the `series_id` doubles as the quote symbol) and `/release/dates` (the economic-release calendar schedule).
The report's FRED series, by Step-3 baseline group:
...
**CFTC.**
The report reads both CFTC datasets' curated bellwether contracts at run level — the full contract set, the two dataset formats, the weekly cadence, the three-week freshness guard, and the change-view exemption are in the [CFTC catalog entry](#cftc-commitments-of-traders).
...
**Economic-calendar consensus + surprise.**
FRED stays the release-schedule backbone; FMP's paid `economic-calendar` (`?country=US&from=&to=`, fields `event` / `date` / `impact` / `previous` / `estimate` / `actual`) layers on two things the FRED schedule can't carry.
(1) For the report's tracked market-moving releases, the engine joins FMP's `estimate` / `actual` onto the matching FRED release through a **curated release→event map** — FRED release names ("Employment Situation") and FMP event names ("Non Farm Payrolls") don't string-match, and one release fans out to several FMP events (Employment Situation alone carries payroll, unemployment-rate, and wage prints, with different units and polarities), so the join is **per event, not per release**.
The map is the **versioned canonical table** the classification codes against: each row is one **release ↔ event pair**, keyed by the **numeric FRED `release_id`** (the shipped `RELEASES` const's stable key, each display name pinned to its ID) plus the FMP event name, the stable per-event identity (a release's rows form its event set).
**Code joins on the numeric ID plus the verified event string, never on the release's display name**, which is presentation text only — the shipped `EconomicRelease` carries only the name downstream today, so the join lives inside the FRED adapter's fetch loop, where the ID is in scope, or the struct gains an ID field, settled at wiring.
Each row carries that event's **unit normalization** between the two feeds, a **date-match tolerance** (same calendar date, ±1 day for cross-midnight publishes — drafted; one shared rule, so it stays out of the per-row columns), a **favorable polarity** (`higher-favorable` / `lower-favorable` / `none` — above-consensus payrolls are favorable; above-consensus CPI is not a "beat"), and an **in-line epsilon** in the event's own units.
The tracked releases are the calendar's seven curated market-moving releases (the shipped `RELEASES` table: Employment Situation, Consumer Price Index, Producer Price Index, Personal Income and Outlays, Advance Monthly Sales for Retail and Food Services, Job Openings and Labor Turnover Survey, Gross Domestic Product — FOMC arrives separately via the FMP `impact` filter below and needs no map row).
The complete row set follows — drafted in-doc and live-verified before wiring (the identity-table convention), the **FMP event strings and raw scales live-verified 2026-07-16**: polarity follows one stated convention — **activity reads are `higher-favorable`, inflation reads `lower-favorable`, and prints whose market read inverts by regime (wages, job openings — growth-positive but rate-hawkish) are `none`** — and the epsilons are judgment-drafted in-line bands like the corpus's other drafted constants; a wrong event string fails soft to an unmapped event (names + dates), never a wrong classification.
Three live-verified name findings ride the join:
- live event names carry a **trailing parenthesized reference period** (`Non Farm Payrolls (Jun)`, `GDP Growth Rate QoQ (Q1)`, weekly prints `(Jul/04)`), so the join matches on the event name **with that trailing token stripped — exact after stripping**, because the feed carries near-names that must not collide (`U-6 Unemployment Rate`; the index-level `CPI` / `Core CPI` / `Producer Price Index` rows);
- the feed carries **duplicate alias rows** for the CPI prints (`Inflation Rate` / `Core Inflation Rate` MoM / YoY mirror the `CPI`-named rows with identical values) — the map keys on the `CPI`-named rows, and the per-event-name dedup keeps the aliases inert;
- a drafted `Core PPI MoM` row was **removed at verification** — the feed carries no core-PPI event in any checked window (only the `Producer Price Index` level / MoM / YoY prints), so the Producer Price Index release maps its headline MoM print alone.

| FRED `release_id` | FRED release (display) | FMP event (live-verified 2026-07-16) | Unit normalization | Polarity | Epsilon |
| --- | --- | --- | --- | --- | --- |
| 50 | Employment Situation | Non Farm Payrolls | thousands of jobs (verified — feed `unit` `K`) | higher-favorable | ±25k |
| 50 | Employment Situation | Unemployment Rate | percent | lower-favorable | ±0.1pp |
| 50 | Employment Situation | Average Hourly Earnings MoM | % month-over-month | none | ±0.1pp |
| 10 | Consumer Price Index | CPI MoM | % month-over-month | lower-favorable | ±0.1pp |
| 10 | Consumer Price Index | Core CPI MoM | % month-over-month | lower-favorable | ±0.1pp |
| 10 | Consumer Price Index | CPI YoY | % year-over-year | lower-favorable | ±0.1pp |
| 46 | Producer Price Index | Producer Price Index MoM | % month-over-month | lower-favorable | ±0.2pp |
| 54 | Personal Income and Outlays | PCE Price Index MoM | % month-over-month | lower-favorable | ±0.1pp |
| 54 | Personal Income and Outlays | Core PCE Price Index MoM | % month-over-month | lower-favorable | ±0.1pp |
| 54 | Personal Income and Outlays | Personal Income MoM | % month-over-month | higher-favorable | ±0.2pp |
| 54 | Personal Income and Outlays | Personal Spending MoM | % month-over-month | higher-favorable | ±0.2pp |
| 9 | Advance Monthly Sales for Retail and Food Services | Retail Sales MoM | % month-over-month | higher-favorable | ±0.3pp |
| 9 | Advance Monthly Sales for Retail and Food Services | Retail Sales Ex Autos MoM | % month-over-month | higher-favorable | ±0.3pp |
| 192 | Job Openings and Labor Turnover Survey | JOLTs Job Openings | millions of openings (verified — feed `unit` `M`) | none | ±0.2M |
| 53 | Gross Domestic Product | GDP Growth Rate QoQ | % annualized quarter-over-quarter | higher-favorable | ±0.3pp |
| 53 | Gross Domestic Product | GDP Price Index QoQ | % annualized quarter-over-quarter | lower-favorable | ±0.2pp |
From each matched event row the engine computes a deterministic, **neutral relation tag** — `above` / `in-line` / `below` estimate (`in-line` when |actual − estimate| ≤ the row's epsilon) — plus the **absolute gap** in the event's own units (**always carried** — the deterministic surprise magnitude even where the % gap is undefined), the signed actual-vs-estimate **% gap** (`(actual − estimate) / |estimate|`, **`None` when the estimate is zero**), and a **beat / miss** read **only where the row maps a polarity** (beat = the favorable side): the engine derives, the model interprets.
The release keeps its single identity: `EconomicRelease` gains a **`surprises` vector — one entry per matched event** (the map's event name as its key, plus estimate / actual / relation tag / absolute gap / % gap / beat-miss), empty when nothing matched and **deduplicated per event name** within the date tolerance (the latest print wins) — so a multi-event release carries every component surprise without overwriting any.
(2) FMP-only **Fed / FOMC events** (filtered to Medium/High `impact`), which FRED has no scheduled-date series for, are appended as a distinct event class, closing the documented FOMC-date gap.
Fail-soft throughout: an unmapped release, a `null` estimate, or an FMP outage leaves that release at **names + dates only** (today's behavior) — never a fabricated consensus.

**Historical sector / industry valuation + performance.**
Today the sector/industry P/E *and* performance are both point-in-time, read only cross-sectionally (which group is rich vs cheap, strong vs weak *right now*).
...
The two series are shaped differently, so the engine derives a different read from each:
...
- the performance endpoints return a daily **`averageChange`** (that date's average constituent move) expressed in **percentage points** (`0.64` means 0.64% — live-verified 2026-07-16, the same convention the shipped snapshot adapter's `change_pct` preserves), *not* a price level or a decimal fraction, so the engine normalizes each day to `daily_return = averageChange / 100` and **compounds the normalized daily changes into a trailing cumulative return** (`Π(1 + averageChange / 100) − 1`, serialized as a decimal fraction) — the rising / falling read — rather than percentiling the raw daily moves (a percentile of `averageChange` would say "today's move was unusual," not "the group is up over the window").

Both derivations run under one **single-homed rule set** (the workflow points here rather than restating it): rows are sorted date-ascending and **deduplicated by date** (the last row for a date wins); a null / non-finite value is dropped, a **P/E ≤ 0 is dropped as inadmissible** against-own-history, and a daily `averageChange` outside **±25 percentage points** is dropped as inadmissible (a unit-error guard — drafted); the **percentile is the inclusive rank** — the share of the window's valid values ≤ the current value, reported 0–100 (ties count as ≤); and a read requires a **minimum sample of ≥ 60 valid points** with the **newest valid point ≤ 7 days old** (both drafted) — below either, the read is **absent plus a typed partial-data gap**, never a numeric default.

Both are compact derived numbers, not the raw series.
Paired, they let the model read a re-rating *with* its price context: a group cheap against its own P/E history *and* up over the trailing window (a re-rating turn) reads differently from one cheap *and still down* (a possible value trap) — the distinction a single snapshot can't support.

**IPO / M&A froth.**
The report has no primary-market feed today, so it can't see the issuance / deal-making pace that runs hot late-cycle and freezes under stress.
Two paid endpoints add it — `ipos-calendar` (`?from=&to=`, recently priced + upcoming scheduled offerings) and `mergers-acquisitions-latest` (`?page=&limit=`, recently announced deals) — which the engine reduces to a compact **activity read**.
That read is the recent-window IPO count and the recent-window M&A deal count, **each recent count paired with its prior equivalent-window count** so the engine carries a native rising / cooling **trend**.
...
Fail-soft and non-floor: a failed gather degrades to no froth signal, never a failed run.
...
- **Persist the derived read, not the raw history.**
  Only the compact derived numbers — the calendar's per-event **`surprises` entries** (event name, estimate, actual, relation tag, **absolute gap**, signed **% gap**, and the polarity-mapped beat / miss read where defined), the P/E percentile + band, the performance trailing cumulative return, and the froth reads (the IPO and M&A **recent + prior-window counts**, the **upcoming-scheduled IPO count**, the bounded notable-name list, and the aggregate proceeds / deal value where the feed carried it) — ride into the packet and the persisted baseline snapshot; the raw ~250-point P/E and performance series and the full IPO / deal lists are transient fetch input, discarded once the derived reads are computed.
  **Every optional derived value has explicit absent semantics** — absent means *not derivable* (an unmapped release, a `null` or zero estimate, an exhausted page budget), **never zero** — so a prompt or snapshot comparison can always distinguish missing from zero.
- **New fields carry `#[serde(default)]`.**
  The enrichments add fields to `BaselineMarketData` and its member structs — `EconomicRelease` (the per-event `surprises` vector, defaulting to empty), `SectorPe` and `IndustrySnapshot` (valuation-history context), `SectorPerformance` and `IndustrySnapshot` (performance-history context) — plus a new issuance-activity group; each must default so an *older* snapshot — serialized before the field existed — still deserializes (to empty / `None`), keeping the prior-vs-current comparison backward-compatible.
  (The prior-snapshot decode is already fail-soft regardless — see [report-workflow.md §Step 3](report-workflow.md#step-3-gather-baseline-market-data).)
- **All stay out of the level-delta engine.**
  The surprise is a native actual-vs-consensus value, the P/E percentile + band and the performance trailing cumulative return are trailing-window structural reads, and the IPO / M&A counts are set-valued activity tallies — none is an inter-report level change — so, like CFTC positioning and the existing movers / earnings / calendar groups, none joins the diffed level groups; the existing point-in-time `sector_pe` level-diff is untouched (it reads only the current `pe`), and the `sectors` performance group stays excluded from the diff exactly as it is today.
...
**Used by:** Portfolio Analysis · Trade Opportunities ([local-models.md](local-models.md)), not the Market Signal Report.
Both suite jobs share the sourcing strategy below; their per-endpoint tables follow.
Beyond the suite-only [Charles Schwab](#charles-schwab-trader-api) / [SEC EDGAR](#sec-edgar) / [FINRA](#finra-short-interest) / [CBOE](#cboe) / [SearXNG](#searxng-local-web-search) sources, the suite also draws on the shared [FMP](#financial-modeling-prep) / [FRED](#fred-federal-reserve-economic-data) / [CFTC](#cftc-commitments-of-traders) catalog entries — the [matrix](#reading-this-document) maps the whole set.

The suite cross-checks company fundamentals against **SEC EDGAR** (keyless), leaving **FMP** for fundamentals breadth, the niche aggregates (movers, earnings calendar, screener, sector / industry P/E), live **quotes** (`quote`), and the deep price history.
The **bulk deep-price load deliberately rides the paid FMP key** (`historical-price-eod/light` in dated form) — the 2026-08-12 ruling that removed the former keyless price rung: an untestable source behind a JS proof-of-work wall was a production risk rather than an asset, and the paid plan's per-minute limit plus the adapter's 429 ladder cover a full run's price load ([verification/2026-08-12-stooq-removal-decision.md](verification/2026-08-12-stooq-removal-decision.md)).
All sit behind the same data-source seam and fail-soft posture as the report's adapters.
...
The **per-symbol** signals here — fundamentals, the revision signal, `financial-scores`, positioning, and peers — are shared by **Portfolio Analysis** (grading held positions) and **Trade Opportunities** (validating candidates), **with one positioning carve-out: the SC 13D/13G activist filings are Trade-Opportunities-only** (a per-candidate accumulation catalyst — Portfolio's endpoint table and its Step-6b positioning layer deliberately exclude them); the **screener / industry-classification** discovery layer is **Trade Opportunities only**, since Portfolio Analysis grades a known holdings list and never screens the universe.
**Short interest** comes keyless from FINRA (FMP has no short-interest endpoint); **cyclical commodity prices** from FRED (daily oil / gas + monthly IMF metals — copper, aluminum, nickel, iron ore, uranium) and FMP's daily commodity series (`HGUSD` / `GCUSD` / `SIUSD` — the identity table in [§Financial Modeling Prep](#financial-modeling-prep)).
**SEC EDGAR is retained as the authoritative primary-source cross-check** for the numbers that drive grades / targets; the ticker→CIK resolver **splits by job** — a **named prerequisite of the full Portfolio slice**, a **non-blocking enhancement for Trade Opportunities' cross-check** (the FMP feeds are symbol-keyed) — per [§SEC EDGAR](#sec-edgar).
DRAM/NAND ASPs and supply-discipline signals have no structured feed and ride the research loop ([web-research.md](web-research.md)).

**Portfolio Analysis** reads a per-holding / per-fund subset of that FMP surface (it grades a known holdings list, so it never touches the discovery layer) plus run-level macro inputs; analyst opinions it pulls — price targets, grade distributions, FMP's ratings snapshot — ride in as *evidence the model weighs, never as inputs to the engine's computed grade*.
Its full endpoint surface, by source, is tabulated under [§Portfolio Analysis — endpoint surface](#portfolio-analysis--endpoint-surface) below.

### Portfolio Analysis — endpoint surface

Portfolio statement and listing unit checks follow [portfolio-analysis.md §Asset eligibility](portfolio-analysis.md#asset-eligibility).
...
*As-built vs designed: the rows the **full-run per-holding pass calls today** are `profile`, the three statement endpoints, `analyst-estimates`, `dividends`, `quote`, `historical-price-eod/light`, and the fund rows below (`etf/info`, the weightings, the sector-P/E pair), plus the run-level `quote` `GCUSD` commodity leg and the sector-benchmark `historical-price-eod/light` legs, so the run's actual FMP call budget is the wired subset alone.
`earnings` is wired as the **quick check's** per-sweep call rather than the full-run per-holding pass, so it sits outside that run budget; the symbol-scoped `news/stock` is wired **both** ways — the quick check's sweep call and the full-run pass's per-stock research-seed pull (the Step-6a seed leg).
Every other row in this table is **designed**, landing with its consumer (the conviction-layer, positioning, and street-opinion reads — [portfolio-analysis.md §The per-holding pipeline](portfolio-analysis.md#the-per-holding-pipeline)).*

| Endpoint path | Cardinality | Portfolio Analysis use |
| --- | --- | --- |
| `profile` | per-holding | sector / industry / **beta** / description — classification + risk input; as-built the wired consumers are the **listing-resolution guard** (issuer name / exchange vs Schwab's instrument identity — [portfolio-analysis.md §Asset eligibility](portfolio-analysis.md#asset-eligibility)) and the outcome episodes' **entry-stamped sector identity** (mapped to its SPDR benchmark — the identity table in [§Financial Modeling Prep](#financial-modeling-prep); [portfolio-analysis.md §Outcome learning](portfolio-analysis.md#outcome-learning-calibration)) — the same read also retains the corporate `website` as a transient eligibility hint for [issuer earnings-release recovery](web-research.md#portfolio-earnings-release-recovery) |
| `income-statement`, `balance-sheet-statement`, `cash-flow-statement` | per-holding | core financial statements — engine fundamentals: the quarterly income prints feed the **TTM statement basis** (four-quarter sums; income/cash-flow TTM derived from the quarterly rows, not a separate `-ttm` call) and the anchor window's trailing driver, the latest **quarterly balance sheet** (one light call) supplies total debt + stockholders' equity ([portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable)) plus, for the pre-profit overlay, **liquid resources alone** — its cash and short-term-investments lines, the four lines that call returns; the overlay's other legs come from the statements that actually carry them: **operating-income eligibility, the two-quarter gross-margin progression and the split-adjusted diluted-share change from the quarterly income prints**, and **TTM cash burn / runway and capex intensity from the quarterly cash-flow prints**; the income and balance-sheet legs re-pulled by the quick check on a fresh filing — as-built no cash-flow re-pull ([portfolio-analysis.md §The quick check](portfolio-analysis.md#the-quick-check-engine-only)) |
...
| `earnings` | per-holding — quick check only | next earnings date + EPS / revenue estimate (catalyst); actual-vs-estimate surprise history — pulled by the quick check (the new-earnings-actual leg), never by the full-run per-holding pass |
...
| `historical-price-eod/light` | per-holding (undated + deep dated forms) + per maturing outcome episode (label time) + run-level (the benchmark legs) | short undated EOD window → momentum / volatility; in **dated form over the deep lookback** it is the suite's **only deep price source** ([verification/2026-08-12-stooq-removal-decision.md](verification/2026-08-12-stooq-removal-decision.md)) → the anchor window's price leg, the drawdown read, and the price-target scenarios; label-time refresh through the matured window end, including exited symbols (cache may substitute only when it covers the full window — [portfolio-analysis.md §Outcome learning](portfolio-analysis.md#outcome-learning-calibration)); the run-level benchmark legs (`^GSPC` + the SPDR sector ETFs — the identity table in [§Financial Modeling Prep](#financial-modeling-prep)) feed the **outcome-learning labels** (built) and the input delta's **technology-event pre-flag** (built — the sector legs fetched run-level per carried holding's sector and memoized; `^GSPC` stays a label-time fetch with no run-level consumer, per [portfolio-workflow.md §Step 5](portfolio-workflow.md)), the per-episode benchmark choice reading each episode's entry-stamped sector identity, never a label-time classification call; the quick check's per-holding price refresh composes quote + dated-EOD and never reads the shared price-bar cache ([portfolio-analysis.md §The quick check](portfolio-analysis.md#the-quick-check-engine-only)) |
...
| `news/stock` | per-holding (full-run seed pull, stocks only) + quick-check sweep | symbol-scoped news headlines — the research-loop **seed** (a lead, never evidence; typed with stable app-assigned IDs — [portfolio-analysis.md §The per-holding pipeline](portfolio-analysis.md#the-per-holding-pipeline)); the quick check additionally pulls it for tech-falsifier holdings (the qualifying-news-seed leg) |
| `etf/info` | per-fund | expense ratio, AUM, NAV, asset class, mandate — refreshed by the quick check (the fund evidence-event leg); serves **closed-end funds an empty body** (probe-verified 2026-08-21 on PDI / GAB / BST), so a CEF carries no NAV, expense, or class from this row |
| `profile` | per-fund | the fund-structure read the closed-end detection consumes — the `isFund` flag plus the description's closed-end fragment, both required ([portfolio-analysis.md §Asset eligibility](portfolio-analysis.md#asset-eligibility) is canonical); full-pass only, never the quick check |
...
Rate prints normalize to **decimal ratios** at the adapter seam before any engine consumption (`4.5` → `0.045`) and persist in that form — the suite's shared rate / return representation, single-homed at [portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable).
...
The whole commodity-context load is built — one **level-basis** date-ranged window per series (never the rate normalization, which would fabricate a price level), each fail-soft to a typed gap counted on data health ([portfolio-workflow.md §Step 5](portfolio-workflow.md#step-5-load-shared-context)).
Each window covers a drafted **~400-day** lookback (`COMMODITY_WINDOW_DAYS`) — wide enough for ~13 monthly IMF prints and a ~1-year daily trend base.

**CFTC** — Socrata, base `https://publicreporting.cftc.gov/resource/<dataset>.json`.
The same keyless pull the report makes; Portfolio Analysis maps a fund holding onto an already-gathered contract.
*Built — the run-level pull reuses the report's COT adapter, and the fund→contract mapping is a deterministic keyword match over the fund's `etf/info` name and asset-class identity (a drafted, calibratable table — `fund::cot_contract_for_fund`) ([portfolio-workflow.md §Step 5](portfolio-workflow.md#step-5-load-shared-context)); the read is layer-(c) positioning evidence, held out of every sub-score.*
...
**Schwab / SEC EDGAR / FINRA / CBOE / web** — the account, keyless, sentiment, and research sources (full endpoint detail in their catalog entries under [The data sources](#the-data-sources) and in [web-research.md](web-research.md)).

| Source · endpoint | Cardinality | Portfolio Analysis use |
| --- | --- | --- |
| Schwab · accounts / positions | run-level | holdings — quantity (signed net), cost basis (app-derived signed total), market value, instrument identity, cash balance, and liquidation value; explicit cash-position rows are reconciled into cash once, with the raw rows retained for audit; the payload carries **no bond analytics and no held-contract greeks** — a limit the future portfolio planner's whole-book reads inherit ([portfolio-analysis.md §Portfolio roll-up](portfolio-analysis.md#portfolio-roll-up)) |
| Schwab · `/marketdata/v1/chains` | per-holding (optionable equity) + per distinct held-contract strike (overlay delta) | option chains → options-activity signal (put/call, IV/skew); **also the built greek source for the same-underlier option overlay** — the overlay's delta comes from a **separate targeted fetch per distinct held strike**, scoped to the held contracts' expiry window, so the activity signal's bounded NTM query is never widened and its whole-chain basis never shifts — a **standalone** option (no held underlier) is a not-rated position that joins no overlay and gets no chain fetch, so its delta is simply absent, never fabricated and never a recorded gap (the class structurally requests nothing — ruled 2026-08-21, this row canonical) |
...
Every endpoint Trade Opportunities ([trade-opportunities.md](trade-opportunities.md)) calls, by source, paralleling the report's and Portfolio Analysis's per-source tables.
Trade Opportunities is a **funnel** (broad discovery → narrowed candidates → expensive per-name validation), so its **cardinality has three bands** rather than Portfolio's two — and the band, not the endpoint, sets the call budget:

- **discovery** — fires a bounded number of times per run and scans the **whole universe** to *generate* candidates (the FMP `company-screener` + event / positioning feeds, the model-led SearXNG research sweep, the short-interest extremes screen — **no `*-bulk` pre-scoring, off-plan**).
  Broad but cheap at this stage: the rich multi-factor signals are **not** computed here — they're spent **per candidate** after the funnel narrows (Step 5c), which is exactly what the discovery-breadth budget rations.
- **per-candidate** — scales with the **narrowed candidate set** and is the **budget driver**: the same per-symbol FMP / SEC surface Portfolio Analysis spends per *holding*, here spent per *candidate*.
  The funnel exists to keep this set small.
...
Three **maintenance populations** additionally ride a subset of the same rows outside the discovery funnel, scaling with the live matrix, watchlist, episode, and archive populations rather than the discovery budget (tagged inline below).
The first is the **carried-matrix / watchlist sweep** — Step 3c's watchlist re-check and Step 7's cheap re-derivation over **one union population, the swept names**: every live matrix carry **plus** every recheckable (`structured`- / `filing`-class) watchlist node — a `research`-class node has no structured feed and therefore never enters this per-symbol sweep — with cache / dedup applied **per distinct symbol after the union**, never to either half alone; the watchlist is independently capped and can exceed the live matrix, so neither half stands in for the other in capacity planning ([configuration.md §Local Analysis Suite Configuration](configuration.md#local-analysis-suite-configuration)).
Separately, Step 3c's capped **research-watchlist refresh lane** selects a drafted **one `research`-class node per DTO run** for a narrow SearXNG / local-model pass using this run's already-fetched structured event seeds; it adds no FMP / SEC sweep row and spends inside the discovery web-fetch / wall-clock ceiling ([trade-opportunities-workflow.md §Step 3c](trade-opportunities-workflow.md#step-3c-carried-forward-watchlist-re-check)).
A swept name spends `analyst-estimates` / `earnings` (both halves' revision-velocity and surprise reads) plus FMP dated-EOD bars through the shared price-bar cache (the carried half's since-flagged read, the watchlist half's relative-strength read); `quote` rides the **carried-matrix half only** (the cheap re-derivation's gate re-runs — watchlist relative strength reads dated-EOD bars, not quotes).
A swept name whose leading metric or a `structured` falsifier is **short-interest-fed** additionally reads the **once-per-run FINRA consolidated file** as a local lookup — the conditional maintenance leg on the FINRA row below: in DTO the Step-3a discovery fetch is reused, in ATO the Quick Audit's Step-2 subset fetches the file conditionally — so the class vocabulary's own flagship `structured` example stays refreshable on every cheap path ([trade-opportunities-workflow.md §Step 3c](trade-opportunities-workflow.md#step-3c-carried-forward-watchlist-re-check)).
The second is the **label-time outcome refresh** — FMP dated-EOD bars through the window end per maturing episode, **picked and shadow alike** (all five turn-away classes — the shadow ledger is plausibly the larger label-time population), counted **per distinct symbol after dedup** through the shared price-bar cache; the per-run population is only the episodes with a window maturing since the last run, never the whole bounded ledger — and the sector-relative legs resolve their benchmark from each episode's **entry-stamped sector identity**, so the label-time surface carries no `profile` / classification request ([trade-opportunities.md §Outcome learning](trade-opportunities.md#outcome-learning-calibration)).
The third is the **archive-price refresh** — FMP dated-EOD bars per **distinct archived symbol** (the departed picks' price-tracked records, capped at 100 — [trade-opportunities.md §Archived opportunities](trade-opportunities.md#archived-opportunities)), counted after dedup against the other populations through the shared price-bar cache (a symbol already swept or label-time-refreshed this run spends nothing again), refreshing each archived pick's since-flagged return and drawdown **every run** — the job-time owner of the archive's "each run" promise, with the render-time floor's lazy rule unchanged for page opens between runs ([storage.md §Local Analysis Suite Storage](storage.md#local-analysis-suite-storage)).
The sweep additionally carries a **conditional filing-cadence rider**: the swept `earnings` row doubles as the fresh-filing trigger, and a **swept name — either half of the union —** whose latest reported period postdates the last sweep refreshes its statement-derived rows — the core statements, `key-metrics` / `ratios`, `financial-scores`, and `financial-growth` (tagged inline below) — for that sweep only, per-name and ~quarterly in practice, so the rider stays small against the per-candidate budget.
The rider is what keeps the cheap sweep's filing-cadence promises honest — the `filing`-class falsifier / leading-metric evaluation, the forensic recomputation, the thin-coverage operating-reality fallback, and the risk tier's statement-side inputs ([trade-opportunities-workflow.md §Step 3c](trade-opportunities-workflow.md#step-3c-carried-forward-watchlist-re-check), [§Step 7](trade-opportunities-workflow.md#step-7-continuity-check--carry-forward)); SEC submissions / company-facts stay per-candidate (the ticker→CIK resolver is non-blocking for this job — [§SEC EDGAR](#sec-edgar)), the symbol-keyed FMP statements being the model-free refresh path.
An ATO **Quick Audit** spends the same per-name maintenance subset over the user-selected names only; a **Deep Audit** spends the full per-candidate surface per selected name, each running the Step-5 loop as a carried-forward candidate ([trade-opportunities-workflow.md §ATO: the audit flow](trade-opportunities-workflow.md#ato-the-audit-flow)).
A failed maintenance retrieval degrades to the typed pending / unavailable label states ([trade-opportunities.md §Outcome learning](trade-opportunities.md#outcome-learning-calibration)) — never a silent stale read.

All FMP paths are on the `https://financialmodelingprep.com/stable` base and run on the shared paid key.
The **per-candidate** per-symbol rows are the same surface tabulated under [§Portfolio Analysis — endpoint surface](#portfolio-analysis--endpoint-surface) (fundamentals, the revision signal, positioning, segments) — repeated here with their Trade-Opportunities use, re-tagged to candidate cardinality, and **minus the ETF / mutual-fund group** (the job hunts operating businesses by archetype, not funds) — **plus the discovery layer Portfolio Analysis never touches** (screener / peers / industry-classification, the structured news / event feeds, and a handful of richer per-candidate signals — growth, activist-stake filings).

*Plan status per endpoint: see [§FMP — current paid-plan tier audit](#fmp--current-paid-plan-tier-audit).
On the current plan **every `*-bulk` endpoint is blocked**, so the universe-wide pre-scoring this layer assumed is gone — the discovery funnel is **redesigned** around `company-screener` + per-symbol scoring on the budget-narrowed candidate slate, plus the model-led hypothesis-research lane (see [trade-opportunities.md](trade-opportunities.md)).
The screener / taxonomy / movers / news-event / insider-latest feeders below all survive.*

**FMP — discovery layer (universe-wide; Trade Opportunities only)**

The `*-bulk` universe-scoring endpoints this layer once leaned on are **off-plan** ([§FMP — current paid-plan tier audit](#fmp--current-paid-plan-tier-audit)); the multi-factor composite + forensic gate they fed move **per-candidate** (Step 5c).
What survives **generates and stratifies** the longlist rather than pre-scoring it:
...
These were **premium-gated on the free tier** (HTTP 402 — see the report's note that FMP's `news/*` feeds are premium) and become available on the suite's **paid** key.
They are a **structured surfacing layer** — ticker-tagged, dated headlines + snippets + source URLs — that *complements*, and does not replace, the keyless SearXNG web-research loop ([web-research.md](web-research.md)): the feed surfaces the headline, the web tool deep-reads the article URL.
They reuse the FMP credential, so they add no new dependency.
The report's own news funnel (FMP Articles) is unchanged; these are suite-only feeders.

*Plan status: see [§FMP — current paid-plan tier audit](#fmp--current-paid-plan-tier-audit); the feeds below are available on the suite's paid key.
Primary-source disclosure catalysts come from `sec-filings-8k` + the web loop.*
...
| `mergers-acquisitions-latest` | discovery (fetched once / run) + per-candidate **lookup** | market-wide M&A feed → takeover-target / deal-flow candidates (discovery); also the per-candidate M&A catalyst read — whether a name is acquirer or target, a local match against the once-fetched feed (never a second request) + `sec-filings-8k` |
...
| `income-statement` (+ `-ttm`), `balance-sheet-statement`, `cash-flow-statement` | per-candidate + per swept name on fresh filing (filing-cadence rider) | core statements — engine fundamentals + forensic divergences (balance-sheet / cash-flow TTM derived from the 4 quarterly statements, not a separate call) |
| `key-metrics`, `ratios` (+ `…-ttm`) | per-candidate + per swept name on fresh filing (filing-cadence rider) | valuation / quality / leverage / margin ratios (archetype-weighted) |
| `financial-scores` | per-candidate + per swept name on fresh filing (filing-cadence rider) | Altman Z + Piotroski → forensic gate |
...
| `dividends` | per-candidate + per maturing picked episode (label time — the supplementary total-return leg; shadow episodes stay price-only) | trailing distributions → the shared v2 scenario-target function's twelve-month payout proxy (not a forward estimate; the function is single-homed at [portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable)); a nonpayer contributes a zero dividend leg (a clean `None`, no gap), and a failed pull is distinguished from it at the producer: at **target time** it reads a zero dividend leg **with a recorded degraded-input gap** — the shared engine's as-built None→zero arithmetic, conservative at the entry gate, the gap feeding the stand-in's flag leg — while at **label time** it costs only the supplementary total-return read, the price-only common basis unaffected (ruled 2026-08-19) |
...
| `quote` | per-candidate + per swept name, carried-matrix half only (Step-7 cheap re-derivation — watchlist relative strength reads dated-EOD bars) + per selected name (ATO Quick Audit) | live quote (current price) |
| `historical-price-eod/light` (dated form) | per-candidate + per swept name (carried-matrix / watchlist sweep, via the shared price-bar cache — both halves) + per maturing picked **or shadow** episode (label time, symbol-deduped) + per **archived pick** (the archive-price refresh, symbol-deduped through the shared cache) + run-level (the commodity series and the benchmark legs — the identity table in [§Financial Modeling Prep](#financial-modeling-prep)) | deep price history → price-action confirmer (relative strength / base breakout), momentum / volatility, scenario targets; the archive leg refreshes only the since-flagged return / drawdown; the run-level commodity series (`HGUSD` / `GCUSD` / `SIUSD`) carry the cyclical sleeve's daily price turn (complementing the monthly IMF series; the gold series pairs the CFTC gold-positioning row); the run-level benchmark legs (`^GSPC` + the SPDR sector ETFs) feed the Step-7 deterministic outcome labels and each carried-forward idea's continuous since-flagged read — the per-name benchmark choice via the record's entry-stamped sector identity, never a label-time classification call ([trade-opportunities.md §Outcome learning](trade-opportunities.md#outcome-learning-calibration)) |

**The segment-acceleration quarterly series is filing-derived.**
FMP's segment endpoints are annual-only (above), which cannot carry the *accelerating*-family gate (seasonally comparable changes, ≥ 5-change minimum history — [trade-opportunities.md §Starting parameters](trade-opportunities.md#starting-parameters-calibratable)).
The quarterly observations come from the name's **own 10-Q / press-release segment disclosures**, deep-read through the bounded research loop and returned as the typed **`leading_metric_observation`** objects of the distillation ([trade-opportunities-workflow.md §Step 5e](trade-opportunities-workflow.md#step-5e-distillation)), which the application validates and **appends per filing** to the candidate's stored leading-metric series before the gate / floor decision (a filing-paced series whose **re-check class is `research`** — the engine cannot refresh it without a model, so a **live opportunity** evaluates only the already-stored series between deep passes, while a worthy-but-unpicked watchlist node may receive the distinct capped targeted refresh above — [trade-opportunities-workflow.md §Step 3c](trade-opportunities-workflow.md#step-3c-carried-forward-watchlist-re-check)).
For an app-eligible new listing / separation, `direct` or explicitly reconciled `recast` limited-history observations from the S-1 / Form 10 path above may join that series only after the identity / perimeter / comparability validation; proxy observations never do.
When the annual history, filing-derived observations, and any valid comparable limited-history observations still do not reach the family minimum, the series is **unmeasurable → evidence-floor abstention** ([trade-opportunities.md §Evidence floor](trade-opportunities.md#evidence-floor)) — never an estimated substitute.

**FRED** — base `https://api.stlouisfed.org/fred`, `/series/observations` (the `series_id` doubles as the quote symbol) plus `/release/dates` (run-level — the **macro-release calendar** schedule, names + dates, that seeds the Step-3b discovery routes; the same free endpoint the report's calendar uses).
Fired once per run as shared context; the commodity set also seeds the commodity-cyclical discovery sleeve (a price turn surfaces names).
Rate prints reach the engine in the suite's shared **decimal-ratio** representation ([portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable)).

| Series ID | Series | Cardinality | Trade Opportunities use |
| --- | --- | --- | --- |
| `DGS10` | 10-Year Treasury Yield | run-level print + one full-run history request | risk-free rate → the v2 scenario-target function's spread-anchored multiples ([portfolio-analysis.md §Starting parameters](portfolio-analysis.md#starting-parameters-calibratable)); the full path loads the same **anchor-window history** per the Portfolio DGS10 row's acquisition rule ([§Portfolio Analysis — endpoint surface](#portfolio-analysis--endpoint-surface)); also loaded by the ATO Quick Audit's target re-anchor under the quick-path cached-print rule ([trade-opportunities.md §Failure posture](trade-opportunities.md#failure-posture)) |
...
**CFTC** — Socrata, base `https://publicreporting.cftc.gov/resource/<dataset>.json`.
The same keyless pull the report and Portfolio Analysis make; Trade Opportunities reads it for the commodity-cyclical sleeve's positioning.
...
**Schwab / SEC EDGAR / FINRA / CBOE / news & web** — the account, keyless, sentiment, and research sources (full endpoint detail in their catalog entries under [The data sources](#the-data-sources) and in [web-research.md](web-research.md)).
...
| SEC EDGAR · submissions (10-K / 10-Q / 8-K; S-1 / Form 10 when app-eligible) | per-candidate | filings — authoritative cross-check + 8-K events, item-classified reads feeding the hard-forensic filing kinds ([trade-opportunities-workflow.md §Step 5c](trade-opportunities-workflow.md#step-5c-deterministic-analysis-archetype-weighted-engine)); for an eligible new listing / separation, S-1 / Form 10 history feeds the limited-history research / validation path, never an automatic series append ([trade-opportunities.md §Evidence floor](trade-opportunities.md#evidence-floor)) (ticker→CIK a non-blocking enhancement — an unresolved CIK reads those kinds unknown, never a silent no-event) |

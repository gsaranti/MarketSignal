...
The application gathers a single bounded page of recent articles per weekly run, alongside Tavily's topic sweep and GDELT's geopolitical query. The feed is best-effort: it is house-written and overlaps what Tavily already indexes, so the deterministic dedup pre-pass and the headline-filter model absorb redundancy, and a failing gather degrades to no headlines rather than failing the job — the same fail-soft posture as GDELT.

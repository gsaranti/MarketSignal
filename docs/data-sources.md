...
FMP, FRED, and CFTC feed **all three jobs**; BLS, GDELT, and the cloud LLMs are report-only; Schwab, SEC EDGAR, Stooq, FINRA, CBOE, and SearXNG are local-suite-only.
...
**Used by:** Portfolio Analysis · Trade Opportunities (local suite only) — deep price history the engine's price-action / momentum / volatility reads need (plus sector / market benchmarks for both jobs' outcome learning and Portfolio's technology-event pre-flag).
Not the report.

Stooq is the suite's **deep historical price source** — keyless, with no documented rate cap, serving 20–30+ years of daily OHLCV per symbol via a simple CSV endpoint (plus bulk database downloads).
Its value holds **independent of FMP's tier**: the multi-decade depth is what the engine's **price-action confirmer** (multi-year base-breakout / long relative-strength) and the momentum / volatility / price-target computations need, and serving that highest-volume per-holding read keylessly keeps the bulk price load off the shared FMP key regardless of cap.
Like GDELT it is best-effort and fail-soft: an informal source with no SLA and occasional symbol-mapping gaps (US tickers take a `.us` suffix), so a missing series degrades to a gap rather than failing the run, and the adapter self-throttles to stay polite (a ≥ 1 s spacing between requests).
Stooq additionally enforces an undocumented **daily-hits limit whose notice arrives as an HTTP 200 HTML body in place of the CSV** — invisible to the status-based retry layer, detected at the parse seam and classified as a typed throttle (live-observed 2026-07-31: 43 of 44 fetches failed this way run-wide).
That notice is not the only 200-HTML body Stooq serves: a **JavaScript proof-of-work browser-verification interstitial** was observed 2026-08-02 answering every non-JS client regardless of headers ([verification/2026-08-02-fmp-light-eod-adjustment-basis.md](verification/2026-08-02-fmp-light-eod-adjustment-basis.md)); the parse seam's classification is deliberately generic — **any** HTML body in the CSV's place trips the same typed throttle and run-wide breaker — so both interstitials degrade identically to the FMP rung, though the throttle's daily-hits label may then misattribute the cause.
Because the limit is account/IP-wide, the first detection trips a **run-wide breaker** — later fetches that run skip Stooq outright instead of burning one doomed request per holding — and the per-holding deep history falls to its second rung: **FMP `historical-price-eod/light` in dated form** over the same lookback serves the anchor window's price leg in Stooq's place (recorded in the audit as a substitution; the dispersal principle holds, since the fallback spends FMP only where Stooq already failed).
Only when both rungs fail does the anchor window starve to its documented raw-percentile / carry fallback, and the run-level data-health roll-up counts every leg ([portfolio-analysis.md §Portfolio roll-up and construction](portfolio-analysis.md#portfolio-roll-up-and-construction)).
Stooq's daily bars are **split-adjusted and dividend-unadjusted** — exactly the price-only basis the suite's labels and relative reads require; the label-time semantics (price-only common basis on both sides, dividends added separately from a label-time `dividends` re-pull) are single-homed in [portfolio-analysis.md §Outcome learning](portfolio-analysis.md#outcome-learning-calibration).
FMP `historical-price-eod/light` serves the **same basis** — split-adjusted, dividend-unadjusted, desk-verified 2026-08-02 against FMP's own `non-split-adjusted` and `dividend-adjusted` variants over the NVDA 2024 split and Altria's cumulative payout ([verification/2026-08-02-fmp-light-eod-adjustment-basis.md](verification/2026-08-02-fmp-light-eod-adjustment-basis.md)) — so the second-rung substitution never mixes adjustment bases.
The suite's benchmark and futures identities are named once, here: the **broad-market benchmark is `^spx`** (the S&P 500 price index), the **sector benchmarks are the SPDR Select Sector ETFs** — ETF proxies rather than S&P sector indices, because Stooq carries the SPDRs' full daily history keylessly — mapped from FMP's sector labels as Technology → `xlk.us`, Financial Services → `xlf.us`, Energy → `xle.us`, Healthcare → `xlv.us`, Industrials → `xli.us`, Consumer Defensive → `xlp.us`, Consumer Cyclical → `xly.us`, Basic Materials → `xlb.us`, Utilities → `xlu.us`, Real Estate → `xlre.us`, Communication Services → `xlc.us`, and the **futures are copper `hg.f`** (high-grade copper, continuous) plus **gold `gc.f` / silver `si.f`** — Trade Opportunities' canonical daily gold / silver price context ([§Trade Opportunities — endpoint surface](#trade-opportunities--endpoint-surface)).
All symbols and the adjustment convention are to be **live-verified on the M5 pre-flight** (Stooq's symbol map is informal); consumers reference this table rather than restating it.
...
The complete row set follows — the Stooq-identities convention (drafted in-doc, live-verified before wiring), the **FMP event strings and raw scales live-verified 2026-07-16**: polarity follows one stated convention — **activity reads are `higher-favorable`, inflation reads `lower-favorable`, and prints whose market read inverts by regime (wages, job openings — growth-positive but rate-hawkish) are `none`** — and the epsilons are judgment-drafted in-line bands like the corpus's other drafted constants; a wrong event string fails soft to an unmapped event (names + dates), never a wrong classification.
...
**Short interest** comes keyless from FINRA (FMP has no short-interest endpoint); **cyclical commodity prices** from FRED (daily energy + monthly IMF metals — copper, aluminum, nickel, iron ore, uranium) and Stooq futures.
...
*As-built vs designed: the rows the job **calls today** are `profile`, the three statement endpoints, `analyst-estimates`, `dividends`, `earnings`, `quote`, `historical-price-eod/light`, `news/stock`, and the fund rows below (`etf/info`, the weightings, the sector-P/E pair); every other row in this table is **designed**, landing with its consumer (the conviction-layer, positioning, and street-opinion reads — [portfolio-analysis.md §The per-holding pipeline](portfolio-analysis.md#the-per-holding-pipeline)), so the run's actual FMP call budget is the wired subset alone.*
...
| `historical-price-eod/light` | per-holding (+ the Stooq-fallback leg where the primary failed) | short undated EOD window → momentum / volatility; in **dated form over the deep lookback** it is the anchor window's **second-rung price source when Stooq is throttled or unavailable** ([§Stooq](#stooq)) |
...
| --- | --- | --- |
| Schwab · accounts / positions | run-level | holdings — quantity (signed net), cost basis (app-derived signed total), market value, instrument identity; the payload carries **no bond analytics and no held-contract greeks**, so a material not-rated position's whole-book contribution is computed from these fields alone — market value + signed notional — with duration / credit / delta riding as typed gaps ([portfolio-analysis.md §Portfolio roll-up and construction](portfolio-analysis.md#portfolio-roll-up-and-construction)) |
| Schwab · `/marketdata/v1/chains` | per-holding (optionable equity) | option chains → options-activity signal (put/call, IV/skew); also the designed greek source for a same-underlier option overlay (the overlay's chain-delta leg lands with the dossier overlay — as-built the Step-7a stand-in classifies without delta) — a **standalone** option (no held underlier) gets no chain fetch, so its delta is a typed gap |
...
| Stooq · daily OHLCV CSV | per-holding + per maturing outcome episode (label time) | deep price history → momentum / volatility / price-target scenarios; throttle-classified with the run-wide breaker and the FMP dated-EOD second rung ([§Stooq](#stooq)); the quick check's per-holding price refresh composes **FMP-only** (quote + dated-EOD) and reads neither Stooq nor the shared cache ([portfolio-analysis.md §The quick check](portfolio-analysis.md#the-quick-check-engine-only)); label-time refresh through the matured window end, including exited symbols (cache may substitute only when it covers the full window — [portfolio-analysis.md §Outcome learning](portfolio-analysis.md#outcome-learning-calibration)) |
| Stooq · daily OHLCV CSV (sector / market benchmarks) | run-level | sector- and market-relative benchmarks (`^spx` + the SPDR sector ETFs — symbols and the FMP-sector mapping single-homed in [§Stooq](#stooq)) → the **outcome-learning labels** (built) and the input delta's technology-event pre-flag (*designed — no input delta exists in code*, per [portfolio-workflow.md §Step 5](portfolio-workflow.md), which already lists that leg as designed); the per-episode benchmark choice reads each episode's entry-stamped sector identity, never a label-time classification call ([portfolio-analysis.md §Outcome learning](portfolio-analysis.md#outcome-learning-calibration)) |
...
| Web tool — keyless SearXNG | per-holding (research lane) + capped pre-loop refresh (≤ 2 holdings / run, drafted) | management commentary from earnings-call transcripts (IR / aggregator sites) + the per-holding research lane for signals with no structured feed, including typed pre-profit production / delivery / bookings / backlog / reservation / guidance / unit-economics observations and the bounded latest-four-period cold-start / gap-fill backfill + the narrow held-name refresh over one named qualitative ledger node; Tavily remains the ordinary fail-soft fallback ([web-research.md](web-research.md), [portfolio-workflow.md §Step 6](portfolio-workflow.md#step-6-per-holding-analysis-loop)) |
...
- **per-candidate** — scales with the **narrowed candidate set** and is the **budget driver**: the same per-symbol FMP / SEC / Stooq surface Portfolio Analysis spends per *holding*, here spent per *candidate*.
...
A swept name spends `analyst-estimates` / `earnings` (both halves' revision-velocity and surprise reads) plus Stooq daily bars through the shared price-bar cache (the carried half's since-flagged read, the watchlist half's relative-strength read); `quote` rides the **carried-matrix half only** (the cheap re-derivation's gate re-runs — watchlist relative strength reads Stooq bars, not quotes).
...
An ATO run spends the same per-name subset over the user-selected names only.
...
| `quote` | per-candidate + per swept name, carried-matrix half only (Step-7 cheap re-derivation — watchlist relative strength reads Stooq bars) + per selected name (ATO Quick Audit) | live quote (current price) |
...
Daily-cadence metals come from Stooq (below) as the higher-frequency complement to the monthly IMF series — copper `hg.f` plus **gold `gc.f` / silver `si.f`**, this job's canonical gold / silver price context ([§Stooq](#stooq); M5 live-verify like the rest of the identity table).
...
| Stooq · daily OHLCV CSV (equities) | per-candidate + per swept name (carried-matrix / watchlist sweep, via the shared price-bar cache — both halves) + per maturing picked **or shadow** episode (label time, symbol-deduped) + per **archived pick** (the archive-price refresh, symbol-deduped through the shared cache) | deep price history → price-action confirmer (relative strength / base breakout), momentum / volatility, scenario targets; the archive leg refreshes only the since-flagged return / drawdown |
| Stooq · daily OHLCV CSV (futures — copper `hg.f`, gold `gc.f`, silver `si.f`; symbols single-homed in [§Stooq](#stooq)) | run-level | daily commodity-price turn for the cyclical sleeve (complements the monthly IMF series; the gold series pairs the CFTC gold-positioning row above) |
| Stooq · daily OHLCV CSV (sector / market benchmarks — `^spx` + the SPDR sector ETFs, [§Stooq](#stooq)) | run-level (outcome learning) | sector- and market-relative forward-return benchmark for the Step-7 deterministic outcome labels and each carried-forward idea's continuous since-flagged read — the per-name benchmark choice via the record's entry-stamped sector identity, never a label-time classification call ([trade-opportunities.md §Outcome learning](trade-opportunities.md#outcome-learning-calibration)) |

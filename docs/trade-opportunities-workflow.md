...
- computes **deterministic outcome labels** on prior picks (realized return vs sector / market, drawdown, whether the leading metric continued, failure mode) for calibration, carries prior opportunities forward with an updated status, and flags any overlap with current holdings
...
- **theme → value chain → exposed names** — tracing each live theme down to its beneficiaries **economically, not just by exposure**: at each node, *who captures the margin* (the chain's economic-profit pool), *who holds the bargaining power* (supplier / buyer concentration), *where capacity is constrained* (the bottleneck conferring pricing power), and *which enabler therefore has real pricing power* rather than only revenue exposure — deliberately past the crowded pure-plays to the **picks-and-shovels enablers** at those margin-capturing, constrained nodes (often mid / small cap), resolved against FMP's industry classification.
...
**Returns.** A schema-validated **forward thematic map** (each live theme with its S-curve position and an **economic value-chain mapping** — per node: margin capture, bargaining power, capacity constraint / bottleneck, and pricing power versus mere exposure) and the **(theme → exposed candidate names)** set — each name with its surfacing rationale and source URLs — flowed into Step 4 alongside the bottom-up screen output. The thematic map is retained as run-level worldview context for the per-candidate thematic-fit read (Step 5d). The model proposes names; it neither fetches per-symbol data nor scores them here.
...
The bounded cap that keeps the per-candidate budget finite is **not a flat top-N by composite score** — a pure score rank collapses the funnel onto whatever is loudest (mega-cap momentum, the most-covered AI names, one crowded macro theme) and throws away the breadth that is the job's edge ([trade-opportunities.md §The research method](trade-opportunities.md#the-research-method)). Instead the slot budget is **allocated under deliberate diversity guardrails**, each a floor-and-ceiling rather than a single ranking:
...
- **feeder** — no single screen (the quant composite, a positioning scan, the theme sweep) may supply more than a set share, so the narrowed set reflects *converging* signals rather than one loud one;
...
Within each axis's floor and ceiling the names are still ranked by **signal strength × house-view fit**; the guardrails shape the *composition* of the narrowed set, never relax the per-name bar, and a candidate displaced by a quota is **recorded, not discarded** (it can resurface in a later run). The result is the narrowed candidate set the expensive per-name loop runs over — the funnel's waist, kept deliberately broad.

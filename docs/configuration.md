...
The choice is the **orchestrator's, made deterministically** from the accumulated evidence-ledger size — never the model's — and four knobs (generous, conservative defaults) apply here: the first three bound the single-vs-hierarchical distillation choice, the last bounds the volume of model-attributed seed provenance:

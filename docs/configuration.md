...
- **Seed-lineage cap** — the maximum number of model-attributed **`seeded_by`** seeds a research stage may record beyond those captured deterministically (a `surfaced_by` tag is stamped free whenever a seed's URL is deep-read and needs no knob; this caps only the seeds the reasoner *names* as orienting a hypothesis without fetching them — most visibly a Trade Opportunities discovery hypothesis card, [web-research.md §The research loop and context management](web-research.md#the-research-loop-and-context-management)).
  The as-built Portfolio default is **4 distinct seed ids per pass**, applied after unknown and duplicate references are dropped, so seed provenance cannot bloat the model's working context; Trade Opportunities adopts the shared knob when its producer lands.

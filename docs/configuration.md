...
The **rotation share and its max-age service level** are themselves configurable, but the slice **cannot be configured off**: whatever the share, it **floors at one slot per run**, so tuning changes the self-refresh's cadence, never voids the bounded-staleness guarantee ([trade-opportunities.md §The two jobs](trade-opportunities.md#the-two-jobs)).

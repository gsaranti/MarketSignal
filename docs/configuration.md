...
That is the matrix's self-refresh, best-effort under the budget with any overdue overflow forming a stalest-first backlog surfaced to the user ([trade-opportunities-workflow.md §Step 4](trade-opportunities-workflow.md#step-4-candidate-consolidation)), so no live name's research can age indefinitely.
...
(Only the *validated* opportunity matrix and this discovery graph carry forward through storage; a name not even worth watchlisting carries no state and is simply re-derivable by a later sweep.)

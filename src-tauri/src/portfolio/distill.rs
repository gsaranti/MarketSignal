...
pub struct DistillInputs<'a> {
    ...
    /// The current ledger's condition ids — `related_condition_id` validation.
    pub ledger_condition_ids: &'a [String],
...
/// the claim. The model never stamps a vintage.
struct Provenance {
...
pub fn distill(model: &dyn DistillModel, inputs: &DistillInputs<'_>) -> Result<DistilledResearch> {
        ...
        for topic in &inputs.research.topics {
            ...
            let body = if own > inputs.input_budget_chars {
                ...
                for (i, pass) in passes.iter().enumerate() {
                    sub_calls_spent += 1;
                    let prompt = pass_prompt(inputs.symbol, &topic.topic_key, i, pass);
                    let body = model
                        .distill_call(
                ...
                tier1_calls += 1;
                let prompt =
                    tree_reduce_prompt(inputs.symbol, &topic.topic_key, &pass_summaries, prior);
                model
                    .distill_call(
                ...
                tier1_calls += 1;
                let prompt = tier1_prompt(inputs.symbol, topic, prior);
                model
                    .distill_call(
    ...
    Ok(validate_combined(wire, inputs, shape, gaps))
}
    ...
    for t in wire.topics {
        ...
        for c in t.claims {
            ...
            let related = c
                ...
                .filter(|id| known_conditions.contains(id.as_str()));
            claims.push(DistilledClaim {
                claim: c.claim,
...
fn render_prior(prior: &TopicDistillate) -> String {
    ...
    for c in &prior.claims {
        out.push_str(&format!("- {} [{}] ({})\n", c.claim, c.source_url, c.vintage));
    }
    out
}

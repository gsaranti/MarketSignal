...
The research messages render on the harness too (`research::samples`, `portfolio-v43`): the gathering and synthesis passes on TSLA's first topic and the synthetic fund's exposure-profile topic over hand-written leads, claims, conditions and pages, in the offline pin `research_messages_are_two_parts_with_no_app_concept` and the dump's last section; the live harness issues no research call, so the research prompts are admitted on the next attempt's first holdings.

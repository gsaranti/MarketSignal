...
**Market-report docs-review pass** (fresh session, cold read, pass 3 of 3): report-workflow.md, report-structure.md, agents.md, thesis-continuity.md, research-documents.md, and the report-relevant sections of the shared docs, under the same review contract, with the two promoted checklist classes from the TO pass. When all three passes are done, `/metis-plan-task` against `docs/portfolio-analysis.md §Asset eligibility` (fund path) — first decision: the fund-form scenario-target methodology. The Codex TO round can land whenever; verify its findings against the docs before agreeing.

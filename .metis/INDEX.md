...
- Scenario-target function (v2 rate-anchored forward multiple — driver ladder × DGS10 spread-anchored P25/50/75 multiples, guards + monotonicity, TR adds fwd dividends, one-month leg v1 mechanics; versioned via the calibration snapshot; as-built v1 drift pending engine update; TO archetype driver overrides; fund-form = the named open item) — portfolio-analysis.md §Starting parameters; trade-opportunities.md §Starting parameters; portfolio-workflow.md §Step 6b; trade-opportunities-workflow.md §Step 5c
- Rate-anchor failure rule (full/deep runs hard-fail pre-per-item after shared retries; engine-only quick paths = cached print ≤ ~1-week max age, `unknown` beyond) — portfolio-analysis.md §Failure posture, §Starting parameters; trade-opportunities.md §Failure posture; data-sources.md §Portfolio Analysis — endpoint surface (FRED), §Trade Opportunities — endpoint surface (FRED)

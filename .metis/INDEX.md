...
- Holding verdict schema — **intrinsic verdict** (grade + sub-scores, conviction, horizon, targets, **standalone action lean**) vs **portfolio action** (fixed ladder + target weight/sizing via the action-sizing spine); what-changed split into intrinsic + action halves — portfolio-analysis.md §The holding verdict
...
- Action-sizing spine (engine bounds the feasible action set from grade/conviction/upside-downside/risk-tier/existing-weight/concentration-headroom/overlap/cash/tax; model proposes within, app constrains) — portfolio-analysis.md §The holding verdict; portfolio-workflow.md §Step 7a
- Position thesis ledger (persisted per-holding standing thesis: thesis + key drivers + **bear/base/bull monitor** + typed quantitative/qualitative **falsifiers** + add/trim/sell **triggers** + target-weight range — the Portfolio analog of TO's opportunity graph; engine evaluates quantitative crossings each run, interpretation rewrites, continuity check validates; fund-flavored for funds) — portfolio-analysis.md §The position thesis ledger; portfolio-workflow.md §Step 6a, §Step 6f, §Step 6g; storage.md §Local Analysis Suite Storage

...
- Source registry & evidence tiers (per-domain metadata — tier 0-5 / deny + **evidenceKinds** (what a source is good for, not one global score) + lanePolicy + credentialMode + freshnessSla + extractionProfile + paywall; a **thin override over heuristic defaults**, not an exhaustive whitelist; unregistered domains scored by default heuristic; tier 0 primary/deterministic → tier 5 sentiment/noise → deny junk) — data-sources.md §Source registry and evidence tiers; web-research.md §Source quality and evidence weighting; configuration.md §Web Research
- Source quality & evidence weighting (engine-computes / model-interprets applied to sourcing; **load-bearing rule — quality informs conviction, never gates discovery** (tiers grade, never exclude; the explicit `deny` list is the one categorical drop), protecting under-covered small/mid-cap breadth; per-document annotation split — app-computed [sourceTier / extractionQuality / recency / primarySourceBonus / paywall-stub] vs model-derived [claimSpecificity / contradiction / supportsClaimIds]; lane policy [discovery soft-prefer, validation stricter weighting, Portfolio→filings/IR, TO→specialist/value-chain]; source-diversity / wire-syndication caps; disconfirming-fetch pass (spent from the existing per-item budget, fail-soft on exhaustion — not an extra pass outside the ceilings); extraction telemetry) — web-research.md §Source quality and evidence weighting
- Connected Sources (optional subscription enrichment — webview login → Keychain domain-scoped session → authenticated fetch → **health test** [connected / connected_but_thin / expired / unsupported]; yield-gated trust (a poorly-rendering paid source isn't silently ranked high); **never on the execution gate**; rides the Schwab credential rails; spend priority Economist / Morningstar / vertical-by-exposure / FT-WSJ) — web-research.md §Connected sources; configuration.md §Connected Sources (subscriptions)
...
- Seed lineage (structured-feed seeds that orient a research loop — FMP news / articles + macro-release calendar — recorded as **leads, never evidence-ledger claims**, preserving surface-not-content; two captures — deterministic **`surfaced_by`** back-pointer stamped on a deep-read claim (≈free, no knob) + bounded model-attributed **`seeded_by`** hypothesis-card field (config-capped, **validated against the app's stable seed-ID set — unknown refs dropped + logged**, so the fabricable leg can't invent lineage); distillation reads the lane as **provenance, never scored evidence**; seed-claim-vs-deep-read divergence = a usable narrative-vs-reality signal; persists onto the opportunity-graph node + the run audit record) — web-research.md §The research loop and context management; trade-opportunities-workflow.md §Step 3b; configuration.md §Research Context Management; storage.md §Local Analysis Suite Storage; trade-opportunities.md §The opportunity, §Signal inputs, §Discovery memory

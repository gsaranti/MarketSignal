...
- Every agent stage sits behind a Rust trait (e.g. `MainAgent`, `AnalystAgent`, `HeadlineFilter`, `ResearchRouter`) whose method takes a typed request and returns a typed, validated response. The model HTTP call is an implementation detail of the adapter, swappable for a deterministic stub.
